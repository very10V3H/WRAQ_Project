        ...
        if (!player.isCreative()) {
            ...
            playerList.forEach(serverPlayer -> {
                if (serverPlayer.getName().getString().equals(name)) {
                    ...
                    player.sendSystemMessage(Component.literal("·治疗强度:").withStyle(CustomStyle.styleOfHealth).withStyle(ChatFormatting.BOLD).append(Component.literal(String.format("%.0f", PlayerAttributes.getHealingAmplification(serverPlayer) * 100)).withStyle(ChatFormatting.WHITE)).append(Component.literal("%").withStyle(ChatFormatting.WHITE)));
