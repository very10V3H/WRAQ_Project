...
public class FireElementSword extends WraqSword implements ActiveItem {
    ...
    public static void Tick(Player player) {
        ...
        if (list.size() > 0)
            BuffSystem.sendEffectLastTime(player, ModItems.FIRE_ELEMENT_SWORD.get().getDefaultInstance(), 8888, Math.min(3, list.size()), true);
        else
            BuffSystem.sendEffectLastTime(player, ModItems.FIRE_ELEMENT_SWORD.get().getDefaultInstance(), 0, Math.min(3, list.size()), true);
    }
