...
public class GuideHud {
    ...
    public static final IGuiOverlay GUIDE_HUD = ((gui, poseStack, partialTick, width, height) -> {
        ...
        if (display && mc.screen == null) {
            if (fun.wraq.process.func.guide.Guide.clientStage == -1 || fun.wraq.process.func.guide.Guide.clientStage >= fun.wraq.process.func.guide.Guide.descriptionMap.size()) return;
            List<Component> components = new ArrayList<>(fun.wraq.process.func.guide.Guide.descriptionMap.get(Guide.clientStage));
