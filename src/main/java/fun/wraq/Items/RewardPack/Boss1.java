...
public class Boss1 extends Item {

    public Boss1(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        level.addParticle(ParticleTypes.FLASH, player.getX(), player.getY(), player.getZ(), 0, 0, 0);
        if (!level.isClientSide && interactionHand == InteractionHand.MAIN_HAND) {
            ...
            ItemStack gemspiece = ModItems.GEM_PIECE.get().getDefaultInstance();
            ItemStack plainsoul = ModItems.PlainSoul.get().getDefaultInstance();
            ItemStack plainrune = ModItems.PlainRune.get().getDefaultInstance();
            if (tmpNum < 5) {
                ...
                while (iterator.hasNext()) {
                    ...
                    TmpPlayer.sendSystemMessage(Component.literal(" " + player.getScoreboardName() + "从Boss奖励包中获得了超级奖励:" + "平原意志*1"));
                }
            } else {
                if (tmpNum < 20) {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag p_41424_) {
        ...
        components.add(Component.literal(" "));
        components.add(Component.literal("BossAward1").withStyle(ChatFormatting.AQUA).withStyle(ChatFormatting.ITALIC));
