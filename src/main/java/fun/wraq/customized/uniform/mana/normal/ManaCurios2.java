...
public class ManaCurios2 extends WraqManaUniformCurios {
    ...
    public static void tick(Player player) {
        if (!isOn(player)) {
            BuffSystem.removeEffectLastTime(player, UniformItems.MANA_CURIOS_2.get());
            return;
        }
        if (playerNearbyHasNoOthers(player))
            BuffSystem.sendEffectLastTime(player, UniformItems.MANA_CURIOS_2.get(), 8888, 0, true);
        else BuffSystem.sendEffectLastTime(player, UniformItems.MANA_CURIOS_2.get(), 0, 0, true);
    }
}
