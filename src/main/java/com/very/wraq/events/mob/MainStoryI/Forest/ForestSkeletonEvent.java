    ...
    public static void ForestSkeleton(TickEvent.LevelTickEvent event) {
        if(event.side.isServer() && event.phase == TickEvent.Phase.START) {
            ...
            for (Skeleton skeleton : Utils.ForestSkeleton) {
                if (skeleton != null && skeleton.isAlive()) Element.ElementProvider(skeleton, Element.Life, 1);
            }
            if(level.getServer().getTickCount() % 300 == 20 && level.equals(level1)) {
                ...
                for(int i=0;i<15;i++)
                {
                    ...
                    if(list0.size() != 0 && (Utils.ForestSkeleton[i] == null || (!Utils.ForestSkeleton[i].isAlive())))
                    {
                        ...
                        Utils.ForestSkeleton[i].getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(30.0D);
                        ...
                        Utils.ForestSkeleton[i].setCustomNameVisible(true);
