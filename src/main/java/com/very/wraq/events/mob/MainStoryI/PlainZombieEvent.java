package com.very.wraq.events.mob.MainStoryI;
    ...
    public static void PlainZombie(TickEvent.LevelTickEvent event) {
        if(event.side.isServer() && event.phase.equals(TickEvent.Phase.START)) {
            ...
            for (Zombie zombie : Utils.PlainZombie) {
                if (zombie != null && zombie.isAlive()) Element.ElementProvider(zombie, Element.Life, 1);
            }
            if (level.getServer().getTickCount() % 300 == 0 && level.equals(level1)
