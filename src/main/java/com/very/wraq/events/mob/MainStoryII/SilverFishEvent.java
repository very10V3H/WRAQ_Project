...
public class SilverFishEvent {
    ...
    public static void SilverFish(TickEvent.LevelTickEvent event) {
        if(event.side.isServer()) {
            ...
            for (Silverfish silverfish : Utils.SilverFish) {
                if (silverfish != null && silverfish.isAlive()) Element.ElementProvider(silverfish, Element.Stone, 1);
            }
            if(level.getServer().getTickCount() % 400 == 60 && level.equals(level1)
                ...
                for(int i=0;i<5;i++)
                {
                    ...
                    if(list0.size() != 0 && (Utils.SilverFish[i] == null || !Utils.SilverFish[i].isAlive()))
                    {
                        ...
                        Utils.SilverFish[i].getAttribute(Attributes.MAX_HEALTH).setBaseValue(9600);
                        Utils.SilverFish[i].getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(100.0D);
                        Utils.SilverFish[i].getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(0.4D);
                        Utils.SilverFish[i].setHealth(9600);
            ...
            if (level.equals(level1) && event.phase.equals(TickEvent.Phase.START)) {
                for (int i = 0; i < 5; i++) {
                    if (Utils.SilverFish[i] != null && Utils.SilverFish[i].isAlive() && level.getServer().getTickCount() % 5 == 0) {
                        List<Player> playerList = level.getEntitiesOfClass(Player.class,AABB.ofSize(Utils.SilverFish[i].position(),5,5,5));
                        for (Player player : playerList) {
                            if (player.position().distanceTo(Utils.SilverFish[i].position()) <= 2.8) {
