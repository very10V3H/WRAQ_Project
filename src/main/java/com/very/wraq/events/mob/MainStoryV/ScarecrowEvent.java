...
public class ScarecrowEvent {
    ...
    public static void Scarecrow(TickEvent.LevelTickEvent event) {
        if (event.side.isServer() && event.phase == TickEvent.Phase.START) {
            ...
            for (Scarecrow scarecrow : Utils.Scarecrow) {
                if (scarecrow != null && scarecrow.isAlive()) Element.ElementProvider(scarecrow, Element.Life, 1.5);
            }
            if(level.getServer().getTickCount() % 400 == 0 && level.equals(level1)) {
                ...
                for (int i=0;i<10;i++) {
                    ...
                    if (list0.size() != 0 && (Utils.Scarecrow[i] == null || !Utils.Scarecrow[i].isAlive())) {
                        ...
                        Compute.SetMobCustomName(Utils.Scarecrow[i], ModItems.ArmorScarecrow.get(),Component.literal("稻草人").withStyle(CustomStyle.styleOfWheat));
