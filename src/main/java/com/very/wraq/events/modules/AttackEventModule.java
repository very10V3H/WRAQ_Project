...
import com.very.wraq.netWorking.misc.ParticlePackets.DragonBreathParticleS2CPacket;
    ...
    public static void ArrowSnowRune3(Player player, CompoundTag data, LivingEntity hurter, Level level) {
        if (data.contains("snowRune") && data.getInt("snowRune") == 3 && data.contains("snowRune3") && data.getInt("snowRune3") == 0) {
            ...
            if (DamageToMob > 0 && (!data.contains("IgnoreRune") || (!data.getBoolean("IgnoreRune"))))
                Compute.FormatMSGSend(player, Component.literal("符石").withStyle(CustomStyle.styleOfRune),
                        Component.literal("冰川符石-冰霜箭矢").withStyle(ChatFormatting.AQUA).
                                append(Component.literal("对怪物造成了").withStyle(ChatFormatting.WHITE)).
                                append(Component.literal(String.format("%.2f", DamageToMob)).withStyle(ChatFormatting.AQUA)).
            ...
            if (DamageToPlayer > 0 && (!data.contains("IgnoreRune") || (!data.getBoolean("IgnoreRune"))))
                Compute.FormatMSGSend(player, Component.literal("符石").withStyle(CustomStyle.styleOfRune),
                        Component.literal("冰川符石-冰霜箭矢").withStyle(ChatFormatting.AQUA).
                                ...
                                append(Component.literal(String.format("%.2f", DamageToPlayer)).withStyle(ChatFormatting.AQUA)).
                                append(Component.literal("伤害值。").withStyle(ChatFormatting.WHITE)));
