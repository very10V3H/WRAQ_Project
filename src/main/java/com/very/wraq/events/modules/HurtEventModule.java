...
public class HurtEventModule {
    ...
    public static void ActionBarNumPacketSender(Player player, Mob monster) {
        ...
        if (data.getDouble(StringUtils.DamageTypes.DamageAmount) > 0) {
/*            String string = "";
            String Crit = " ";
            if (data.getBoolean(StringUtils.DamageTypes.Crit)) Crit = Utils.Emoji.CritRate;
            if (data.getDouble(StringUtils.DamageTypes.DamageIgnoreDefenceAmount) > 0) {
                string = "+ [" + String.format("%.1f",data.getDouble(StringUtils.DamageTypes.DamageIgnoreDefenceAmount)) + "]";
            }
            if ((data.contains(StringUtils.DamageTypes.IsAttack) && data.getBoolean(StringUtils.DamageTypes.IsAttack))) {
                ClientboundSetActionBarTextPacket clientboundSetActionBarTextPacket =
                        new ClientboundSetActionBarTextPacket(Component.literal(Crit).withStyle(CustomStyle.styleOfPower).
                                append(Component.literal(String.format("%.1f",data.getDouble(StringUtils.DamageTypes.DamageAmount)) + " ").withStyle(ChatFormatting.YELLOW)).
            ...
            if ((data.contains(StringUtils.DamageTypes.IsMana) && data.getBoolean(StringUtils.DamageTypes.IsMana))) {
                ClientboundSetActionBarTextPacket clientboundSetActionBarTextPacket =
                        new ClientboundSetActionBarTextPacket(Component.literal(Crit).withStyle(CustomStyle.styleOfEntropy).
                                append(Component.literal(String.format("%.1f",data.getDouble(StringUtils.DamageTypes.DamageAmount)) + " ").withStyle(ChatFormatting.LIGHT_PURPLE)).
                                append(Component.literal(string).withStyle(CustomStyle.styleOfSea)));
                ServerPlayer serverPlayer = (ServerPlayer) player;
                serverPlayer.connection.send(clientboundSetActionBarTextPacket);
            }*/
    ...
    public static void VolcanoRune0Judge(Player player) throws IOException {
        CompoundTag data = player.getPersistentData();
        if (data.contains("volcanoRune") && data.getInt("volcanoRune") == 0) {
            Compute.PlayerHeal(player,(player.getMaxHealth() - player.getHealth()) * 0.12f * Compute.EndRune3Judge(player,1));
            ItemStack itemStack = ModItems.SilverCoin.get().getDefaultInstance();
            itemStack.setCount(Compute.EndRune3Judge(player,1));
            Compute.ItemStackGive(player,itemStack);
        }
    }
/*    public static void VolcanoRune2Judge(Player player) {
        CompoundTag data = player.getPersistentData();
    ...
    public static void ManaSkill14 (CompoundTag data, Player player) {
        ...
        if (Compute.ManaSkillLevelGet(data,14) > 0 && data.getInt(StringUtils.ManaSkillNum.Skill14) < TickCount) {
            ...
            for (Player player1 : playerList)
                if (player1 != player) Compute.Damage.ManaDamageToPlayer(player, player1, Compute.ManaSkillLevelGet(data, 14) * Count.get());
            ServerPlayer serverPlayer = (ServerPlayer) player;
