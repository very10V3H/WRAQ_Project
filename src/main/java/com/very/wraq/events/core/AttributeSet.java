...
public class AttributeSet {
    ...
    public static void SpeedSet(TickEvent.PlayerTickEvent event) {
        if (event.side.isServer() && event.phase.equals(TickEvent.Phase.START)) {
            ...
            double SpeedUp = Compute.PlayerAttributes.PlayerMovementSpeed(player);
            ...
            if (!player.isDeadOrDying()) {
                ...
                double ManaUp = Compute.PlayerAttributes.PlayerMaxMana(player);
                data.putDouble("MAXMANA", 100 + ManaUp);
            } // 生命回复与魔力回复

            //最大生命值、魔力、攻击距离修改。
            double MaxHealth = Compute.PlayerAttributes.PlayerMaxHealth(player) + 200;
            ...
            if (data.contains("MANA") && data.contains("MAXMANA")) {
                ...
                double ManaRecover = 5 + Compute.PlayerAttributes.PlayerManaRecover(player);
            ...
            if (data.contains(StringUtils.Swift) && data.contains(StringUtils.MaxSwift)) {
                Compute.PlayerSwiftChange(player,(5 + Math.min(5, Compute.PlayerAttributes.PlayerExtraSwiftness(player))) / 20);
            }
        }
    }
}
