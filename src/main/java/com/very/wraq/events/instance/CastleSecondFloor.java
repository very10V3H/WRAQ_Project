...
public class CastleSecondFloor {
    ...
    public static void PlayerPickItemExDamage(Player player, Mob mob) {
        if (!mob.getItemBySlot(EquipmentSlot.HEAD).is(ModItems.MobArmorCastleKnightHelmet.get()) || mob instanceof Civil) return ;
    ...
    public static double MobDamageImmune(Player player, Mob mob) {
        if (!mob.getItemBySlot(EquipmentSlot.HEAD).is(ModItems.MobArmorCastleKnightHelmet.get()) || mob instanceof Civil) return 1;
        if (shieldCount > 0) {
            Compute.FormatMSGSend(player,Component.literal("暗黑城堡").withStyle(CustomStyle.styleOfCastle),
                    Component.literal("暗黑骑士还有" + shieldCount + "层顽盾!").withStyle(ChatFormatting.WHITE));
        ...
        if (event.side.isServer() && event.phase.equals(TickEvent.Phase.START)
            ...
            if (Utils.instanceList.get(InstanceIndex).isChallenge()) {
                ...
                switch (instance.getDifficulty()) {
                    case 0 ->
                            difficulty = Component.literal("一般").withStyle(ChatFormatting.GREEN);
                    case 1 ->
                            difficulty = Component.literal("困难").withStyle(ChatFormatting.LIGHT_PURPLE);
                    case 2 ->
                            difficulty = Component.literal("炼狱").withStyle(ChatFormatting.RED);
                }
