...
public class FireElementSword extends WraqSword implements ActiveItem {
    ...
    public static void Tick(Player player) {
        ...
        if (list.size() > 0)
            Compute.effectLastTimeSend(player, ModItems.FireElementSword.get().getDefaultInstance(), 8888, Math.min(3, list.size()), true);
        else
            Compute.effectLastTimeSend(player, ModItems.FireElementSword.get().getDefaultInstance(), 0, Math.min(3, list.size()), true);
    }
