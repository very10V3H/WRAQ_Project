package com.very.wraq.process.damage;
                ...
                if (this.generation == 3) {
                    nextCauseMob.forEach(mob -> {
                        if (!causedDamageMob.contains(mob) && !mob.equals(this.originMob)) {
                            if (this.damageType == 0) Compute.Damage.AttackDamageToMonster_AdDamage_Direct(originPlayer, mob, this.originDamage / (2 * (this.generation + 1)));
                            if (this.damageType == 1) Compute.Damage.ManaDamageToMonster_ApDamage_Direct(originPlayer, mob, this.originDamage / (2 * (this.generation + 1)));
                            if (this.damageType == 2) Compute.Damage.IgnoreDefenceDamageToMonster_Direct(originPlayer, mob, this.originDamage / (2 * (this.generation + 1)));
                        }
