...
public class AttackAnimationS2CPacket {
    private final int playerId;
    private final int count;

    public AttackAnimationS2CPacket(int playerID, int count) {
        this.playerId = playerID;
        this.count = count;
    }

    public AttackAnimationS2CPacket(FriendlyByteBuf buf) {
        ...
        context.enqueueWork(() -> {
            ...
            if (entity instanceof Player) {
                ...
                if (animation != null) {
                    switch (count) {
                        case 0 -> {
                            animation.setAnimation(new KeyframeAnimationPlayer(PlayerAnimationRegistry.getAnimation(new ResourceLocation(Utils.MOD_ID, "test_attack1"))));
                        }
                        case 1 -> {
