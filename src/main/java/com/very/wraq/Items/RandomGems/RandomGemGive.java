...
public class RandomGemGive extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            ...
            Compute.RandomAttributeProvider(itemStack,1,1,player);
            Compute.RandomAttributeProvider(itemStack,1,1,player);
            Compute.FormatBroad(player.level(), Component.literal("BOSS").withStyle(ChatFormatting.DARK_RED),
                    Component.literal(player.getName().getString()+"通过四元饰品袋").withStyle(ChatFormatting.WHITE).
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag flag) {
        components.add(Component.literal("奖励包-饰品").withStyle(ChatFormatting.LIGHT_PURPLE));
        components.add(Component.literal("右键获取随机四元饰品。").withStyle(ChatFormatting.WHITE));
