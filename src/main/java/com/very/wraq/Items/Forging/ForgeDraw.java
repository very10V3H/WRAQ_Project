 ...
 public class ForgeDraw extends Item {
     ...
     public ForgeDraw(Properties p_41383_, Item item) {
         ...
         if (Utils.MainHandTag.containsKey(item) || Utils.OffHandTag.containsKey(item)) Utils.WeaponList.add(this);
         if (Utils.ArmorTag.containsKey(item)) Utils.ArmorList.add(this);
         if (Utils.CuriosTag.containsKey(item)) Utils.CuriosList.add(this);
     }
     ...
     public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
         ...
         if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND))
         {
             ...
             if(ContainMaterial) {
                 ...
                 Compute.FormatBroad(level,Component.literal("锻造").withStyle(ChatFormatting.GRAY),
                         Component.literal(player.getName().getString()+"成功锻造了 ").withStyle(ChatFormatting.WHITE)
                                 .append(itemStack.getDisplayName())
                                 .append(Component.literal(" 这是维瑞阿契的第").withStyle(ChatFormatting.WHITE))
                                 .append(Component.literal("" + Utils.forgeNumMap.get(ForgedItem)).withStyle(ChatFormatting.AQUA))
                 ...
                 ClientboundSetSubtitleTextPacket clientboundSetSubtitleTextPacket =
                         new ClientboundSetSubtitleTextPacket(Component.literal("已被" + player.getName().getString() + "成功锻造！").withStyle(ChatFormatting.AQUA));
             ...
             else
             {
                 ...
                 for (int i = 0; i < MaterialList.size(); i ++) {
                     if (Compute.ItemStackCount(inventory,MaterialList.get(i).getItem()) < MaterialList.get(i).getCount()) {
                         Compute.FormatMSGSend(player,Component.literal("锻造").withStyle(ChatFormatting.GRAY),
                                 Component.literal("缺少:").withStyle(ChatFormatting.WHITE).
                 ...
                 Compute.FormatMSGSend(player,Component.literal("锻造预览").withStyle(ChatFormatting.GRAY),
                         Component.literal("锻造预览：").withStyle(ChatFormatting.WHITE).
                                 append(ForgedItem.getDefaultInstance().getDisplayName()));
             }
         }
