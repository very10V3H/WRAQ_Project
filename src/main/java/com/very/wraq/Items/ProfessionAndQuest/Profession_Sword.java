...
public class Profession_Sword extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        if(!level.isClientSide && interactionHand == InteractionHand.MAIN_HAND)
        {
            ...
            {
                if(data.contains("Sword")) player.sendSystemMessage(Component.literal("当前剑士职业击杀数:").withStyle(ChatFormatting.GOLD).append(Component.literal(String.valueOf(data.getInt("Sword")))));
                else
                {
                    data.putInt("Sword",0);
