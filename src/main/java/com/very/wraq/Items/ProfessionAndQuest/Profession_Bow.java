...
public class Profession_Bow extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        if(!level.isClientSide && interactionHand == InteractionHand.MAIN_HAND)
        {
            ...
            {
                if(data.contains("Bow")) player.sendSystemMessage(Component.literal("当前弓箭手职业击杀数:").withStyle(ChatFormatting.GOLD).append(Component.literal(String.valueOf(data.getInt("Bow")))));
                else
                {
                    data.putInt("Bow",0);
