package com.very.wraq.series.overWorld.MainStory_I.Plain.Runes_Plain;
...
public class GreenRunes_3 extends Item {
    public GreenRunes_3(Item.Properties p_41383_) {
        super(p_41383_);
    }
    @Override
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag p_41424_) {
        ...
        components.add(Component.literal("每过10s，抵御一次攻击。"));
        Compute.DescriptionDash(components,ChatFormatting.WHITE,ChatFormatting.GREEN,ChatFormatting.WHITE);
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand)
    {
        if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            player.getPersistentData().putInt("Green",3);
            player.getPersistentData().putInt("Green3",0);
            Compute.FormatMSGSend(player,Component.literal("符石").withStyle(Style.EMPTY.withColor(TextColor.parseColor("#bfbcbc"))),
                    Component.literal("平原符石-平原加护").withStyle(ChatFormatting.GREEN).append(Component.literal("已激活").withStyle(ChatFormatting.WHITE)));
