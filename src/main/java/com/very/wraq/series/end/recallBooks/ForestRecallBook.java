...
public class ForestRecallBook extends Item {
    public ForestRecallBook(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag p_41424_) {
        ...
        components.add(Component.literal("森林的回忆").withStyle(ChatFormatting.DARK_GREEN));
        components.add(Component.literal(" "));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if (!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            if (Compute.RecallPlayerCheck((ServerPlayer) player)) {
                ...
                if (Utils.forestRecall.recallPlayer == null && Utils.forestRecall.recallCount == -1 && level.equals(level.getServer().getLevel(Level.END))) {
                    Compute.playerItemUseWithRecord(player);
                    Utils.forestRecall.recallCount = 620;
                    Utils.forestRecall.recallPlayer = (ServerPlayer) player;
                    Utils.forestRecall.recallTimeLimit = 12000;
                    Utils.forestRecall.playerName = player.getDisplayName();
                    Utils.forestRecall.style = CustomStyle.styleOfForest;
                    Utils.forestRecall.zoneName = "森林";
                } else {
