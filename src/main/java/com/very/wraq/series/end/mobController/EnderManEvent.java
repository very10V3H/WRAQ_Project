...
public class EnderManEvent {
    ...
    public static void EnderMan(TickEvent.LevelTickEvent event)
    {
        if (event.side.isServer())
        {
            ...
            if(level.getServer().getTickCount() % 200 == 80 && level.equals(level1)
...
/*                if (list.size() > 40) {
                ...
                for(int i=0;i<4;i++)
                {
                    if(list0.size() != 0 && (Utils.EnderMan[i] == null || !Utils.EnderMan[i].isAlive()))
                    {
                        ...
                        Utils.EnderMan[i].setCustomName(Component.literal("终界使者").withStyle(CustomStyle.styleOfEnd));
            ...
            if (level.equals(level1) && event.phase.equals(TickEvent.Phase.START)) {
                for (int i = 0; i < 4; i++) {
                    if (Utils.EnderMan[i] != null && Utils.EnderMan[i].isAlive() && level.getServer().getTickCount() % 5 == 0) {
                        List<Player> playerList = level.getEntitiesOfClass(Player.class,AABB.ofSize(Utils.EnderMan[i].position(),5,5,5));
                        for (Player player : playerList) {
                            if (player.position().distanceTo(Utils.EnderMan[i].position()) <= 2.8) {
