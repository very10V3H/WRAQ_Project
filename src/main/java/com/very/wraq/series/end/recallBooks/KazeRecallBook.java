...
public class KazeRecallBook extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag p_41424_) {
        ...
        components.add(Component.literal("狂风谷地的回忆").withStyle(CustomStyle.styleOfKaze));
        components.add(Component.literal(" "));
        ...
        if (!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            if (Compute.RecallPlayerCheck((ServerPlayer) player)) {
                ...
                if (Utils.KazeRecall.RecallPlayer == null && Utils.KazeRecall.RecallCount == -1 && level.equals(level.getServer().getLevel(Level.END))) {
                    ...
                    Utils.KazeRecall.RecallCount = 620;
                    Utils.KazeRecall.RecallPlayer = (ServerPlayer) player;
                    Utils.KazeRecall.RecallTimeLimit = 12000;
                    Utils.KazeRecall.playerName = player.getDisplayName();
                    Utils.KazeRecall.style = CustomStyle.styleOfKaze;
                    Utils.KazeRecall.zoneName = "风之谷";
                }
                else {
