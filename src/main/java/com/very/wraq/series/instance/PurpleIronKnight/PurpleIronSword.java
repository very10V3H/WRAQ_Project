...
public class PurpleIronSword extends SwordItem{
    ...
    public static double DamageEnhance(Player player, Mob mob) {
        ...
        for (int i = 3 ; i < 9 ; i ++) {
            if (inventory.getItem(i).getItem() instanceof PurpleIronSword
                    || inventory.getItem(i).getItem() instanceof PurpleIronBow
                    || inventory.getItem(i).getItem() instanceof PurpleIronSceptre ) isOn = true;
        }
        if (!isOn) return 0;
