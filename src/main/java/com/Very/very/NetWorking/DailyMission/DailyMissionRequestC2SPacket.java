                    ...
                    if (allowToRequestTime.before(Calendar.getInstance())) {
                        ...
                        Compute.FormatMSGSend(serverPlayer, Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze),
                                Component.literal("你成功接取了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                        append(Component.literal(" 每日任务 !").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze)));
                    }
                    else {
                        Compute.FormatMSGSend(serverPlayer, Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze),
                                Component.literal("还未到达可以接取每日任务的时间。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                    }
                }
                else {
                    Compute.FormatMSGSend(serverPlayer, Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze),
                            Component.literal("每日任务需要达到20级才能开始接取。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                }
            }
            else {
                Compute.FormatMSGSend(serverPlayer, Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze),
                        Component.literal("你已经接取每日任务了！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
