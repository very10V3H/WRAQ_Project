            ...
            if (!Utils.playerReputationMissionContent.containsKey(serverPlayer.getName().getString())) {
                ...
                if (allowToRequest) {
                    if (serverPlayer.experienceLevel >= 20) {
                        ...
                        Utils.playerReputationMissionContentNum.put(serverPlayer.getName().getString(),random.nextInt(40, (int) Math.max(41,serverPlayer.experienceLevel * 1.8)));
                        ...
                        Compute.FormatMSGSend(serverPlayer, Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze),
                                Component.literal("你成功接取了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                        append(Component.literal(" 悬赏任务 !").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GOLD)));

                    }
                    else {
                        Compute.FormatMSGSend(serverPlayer, Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze),
                                Component.literal("悬赏任务需要达到20级才能开始接取。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                    }
                }
                else {
                    Compute.FormatMSGSend(serverPlayer, Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze),
                            Component.literal("悬赏任务惩罚尚未结束。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));

                }
            }
            else {
                Compute.FormatMSGSend(serverPlayer, Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze),
                        Component.literal("你已经接取悬赏任务了！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            }
        });
        return true;
    }
