                    ...
                    if (!Utils.playerTeamMap.containsKey(player)) {
                        if (playerTeam.teamPlayerNum() < 8) {
                            if (!playerTeam.containPlayer(player)) {
                                ...
                                player.sendSystemMessage(Component.literal("您已加入").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                        ...
                        if (!playerTeam.containPlayer(player)) teamLeader.sendSystemMessage(Component.literal("该玩家已在其他队伍内。"));
                    }
