            ...
            switch (flag){
                case 0 ->{
                    if(data.contains("security0") && data.getDouble("security0") >= 1){
                        ...
                        if(data.contains("securityGet")) data.putDouble("securityGet",data.getDouble("securityGet")+Utils.security0*0.95f);
                        ...
                        Compute.FormatBroad(player.level(), Component.literal("股票").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GOLD),
                                Component.literal(player.getName().getString()+"出售了价值"+String.format("%.2f",Utils.security0)+"的维瑞库尤酒店股票。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GREEN).
                                        ...
                                        append(Component.literal(String.format("%.2f",data.getDouble("securityGet"))).withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA)));
                    }
                    else{
                ...
                case 1 -> {
                    if(data.contains("security1") && data.getDouble("security1") >= 1){
                        ...
                        if(data.contains("securityGet")) data.putDouble("securityGet",data.getDouble("securityGet")+Utils.security1*0.95f);
                ...
                case 2 -> {
                    if(data.contains("security2") && data.getDouble("security2") >= 1){
                        ...
                        if(data.contains("securityGet")) data.putDouble("securityGet",data.getDouble("securityGet")+Utils.security2*0.95f);
                ...
                case 3 -> {
                    ...
                    data.putDouble("security3",data.getDouble("security3")-1);
                        if(data.contains("securityGet")) data.putDouble("securityGet",data.getDouble("securityGet")+Utils.security3*0.95f);
