    ...
    public boolean handle(Supplier<NetworkEvent.Context> supplier)
    {
        ...
        context.enqueueWork( ()->{
            ...
            if (teamLeader != null) {
                ...
                if (!Compute.thisTeamIsChallenging(playerTeam)) {
                    if (playerTeam.teamPlayerNum() < 8) {
                        if (!playerTeam.containPlayer(player)) {
                            ...
                            player.sendSystemMessage(Component.literal("您加入了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                ...
                else player.sendSystemMessage(Component.literal("该队伍正在挑战副本，请等待副本挑战结束后再尝试加入。"));
            }
        });
        return true;
    }
