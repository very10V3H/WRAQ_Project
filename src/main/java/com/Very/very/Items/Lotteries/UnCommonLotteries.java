package com.Very.very.Items.Lotteries;
...
public class UnCommonLotteries extends Item {

    public UnCommonLotteries(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    private void setItemAndWeightList() {
        itemAndWeightList.add(new ItemAndWeight(ModItems.FantasyMedal.get(),2));
        itemAndWeightList.add(new ItemAndWeight(ModItems.FantasyBracelet.get(),2));
        itemAndWeightList.add(new ItemAndWeight(ModItems.WorldSoul3.get(),2));
        itemAndWeightList.add(new ItemAndWeight(ModItems.ForgeImprove2.get(),20));
        itemAndWeightList.add(new ItemAndWeight(ModItems.ForgeEnhance2.get(),20));
        itemAndWeightList.add(new ItemAndWeight(ModItems.ReputationMedal.get(),20));
        itemAndWeightList.add(new ItemAndWeight(ModItems.CompleteGem.get(),20));
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level p_41422_, List<Component> components, TooltipFlag flag) {
        ...
        components.add(Component.literal("从彩券中，你可能会得到:").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFantasy));
        itemAndWeightList.forEach(itemAndWeight -> {
        ...
        if (!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            ...
            itemAndWeightList.forEach(itemAndWeight -> {
                if (RandomIndex.get() > 0) {
                    ...
                    if (RandomIndex.get() <= 0) {
                        if (itemAndWeight.getWeight() < 10) Compute.FormatBroad(level, Component.literal("彩券").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFantasy),
