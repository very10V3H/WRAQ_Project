package com.Very.very.Items.Mission;
...
public class Daily extends Item {
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        components.add(Component.literal("-每日任务").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA));
...
*/
            if(!ItemData.getString("Owner").equals(player.getName().getString())){
                ...
                for(int i = 1; i <= 17; i++)
                {
                    if(ItemData.contains("DailyMission"+i) && data.contains("DailyMission"+i) && data.getInt("DailyMission"+i) < ItemData.getInt("DailyMission"+i)) flag = false;
                    if(!flag) break;
                }
                if(!flag)
                {
                ...
                {
                    ItemStack GemPiece = Moditems.gemspiece.get().getDefaultInstance();
                    ...
                    Compute.FormatBroad(level,Component.literal("每日任务").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA),
                            Component.literal(player.getName().getString()+"完成了每日任务！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
