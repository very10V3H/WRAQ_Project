...
public class openSecurity extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        {
            if(player.getPersistentData().contains("PersistentSecurity0")) Utils.security0 = player.getPersistentData().getDouble("PersistentSecurity0");
            if(player.getPersistentData().contains("PersistentSecurity1")) Utils.security1 = player.getPersistentData().getDouble("PersistentSecurity1");
            if(player.getPersistentData().contains("PersistentSecurity2")) Utils.security2 = player.getPersistentData().getDouble("PersistentSecurity2");
            if(player.getPersistentData().contains("PersistentSecurity3")) Utils.security3 = player.getPersistentData().getDouble("PersistentSecurity3");
