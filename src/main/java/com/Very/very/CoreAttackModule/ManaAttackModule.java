...
import com.Very.very.Series.InstanceSeries.Moon.Equip.MoonBook;
import com.Very.very.Series.InstanceSeries.Moon.Equip.MoonSceptre;
import com.Very.very.Series.InstanceSeries.Moon.MoonCurios;
import com.Very.very.Series.OverWorldSeries.Castle.TreeBracelet;
...
public class ManaAttackModule {
    public static void BasicAttack(Player player, Entity entity, double BaseDamage, double BreakDefence, double BreakDefence0, Level level, Entity Arrow) {
        ...
        if (entity instanceof Mob monster && !(entity instanceof Villager)) {
            ...
            double HealthSteal = Compute.PlayerManaHealSteal(player);
            ...
            // display
            if (IsCrit) Compute.SummonValueItemEntity(monster.level(), player, monster, Component.literal(String.format("%.0f", Damage + DamageIgnoreDefence)).withStyle(Utils.styleOfEntropy));
            else Compute.SummonValueItemEntity(monster.level(), player, monster, Component.literal(String.format("%.0f", Damage + DamageIgnoreDefence)).withStyle(Utils.styleOfMana));
            ...
            CastleSceptre.NormalAttack(player,monster,Damage);
        ...
        if (entity instanceof Player hurter) {
            ...
            double HealSteal = Compute.PlayerManaHealSteal(player);
            ...
            damage = BaseDamage * (1.0d - (0.25F * log(((Defence) * (1.0d - BreakDefence)) * (E * E / 250) + 1)));
    ...
    public static void ManaSkill13Attack(CompoundTag data, Player player) {
        if (Compute.ManaSkillLevelGet(data, 13) > 0 && Utils.ManaSkill13.containsKey(player)
            ...
            Compute.PlayerManaAddOrCost(player, (int) (data.getInt("MAXMANA") * 0.2 * Compute.ManaSkillLevelGet(data, 13)));
    ...
    public static double ManaSkill10(Player player) {
        ...
        if (Compute.ManaSkillLevelGet(data, 10) > 0 && Utils.SceptreTag.containsKey(mainhand)) {
            return Compute.PlayerAttributes.PlayerCritDamage(player)  * Compute.ManaSkillLevelGet(data, 10) * 0.15;
        } // 法术专精-力凝魔核
        return 0;
    }
    ...
    public static double ManaSkill10DamageEnhance(Player player) {
        ...
        if (Compute.ManaSkillLevelGet(data,10) > 10 && Utils.SceptreTag.containsKey(player.getMainHandItem().getItem())) {
            return Compute.PlayerAttributes.PlayerCoolDownDecrease(player) * 0.25;
        }
        return 0;
    }
    ...
    public static double ManaSkill11(Player player) {
        ...
        if (Compute.ManaSkillLevelGet(data, 11) > 0 && Utils.SceptreTag.containsKey(mainhand)) {
            return Compute.ManaSkillLevelGet(data, 11) * 0.2;
        } // 法术专精-术法全析
        return 0;
    }

    public static double NetherManaArmor(Player player, Mob mob) {
        return Compute.ArmorCount.NetherMana(player) * 0.35 * Compute.MonsterManaDefence(mob) / 500.0;
    }
    ...
    public static double EndRune1(Player player, double BaseDamage) {
        ...
        if (data.getInt(StringUtils.EndRune) == 1 && (!Utils.EndRune1CoolDown.containsKey(player)
            ...
            if (ManaCost > 0) {
                ...
                return BaseDamage * ((double) Compute.PlayerCurrentManaNum(player) / ManaCost);
            }
        }
        return 0;
    }
