...
import com.Very.very.Events.Instance.PurpleIronKnight;
...
import com.Very.very.NetWorking.Packets.AnimationPackets.AnimationTickResetS2CPacket;
...
import com.Very.very.NetWorking.Packets.TeamPackets.ScreenSetS2CPacket;
import com.Very.very.NetWorking.Packets.TeamPackets.TeamInfoResetS2CPacket;
import com.Very.very.NetWorking.Reputation.ReputationValueS2CPacket;
import com.Very.very.NetWorking.ReputationMission.ReputationMissionAllowRequestTimeS2CPacket;
import com.Very.very.NetWorking.ReputationMission.ReputationMissionContentS2CPacket;
import com.Very.very.NetWorking.ReputationMission.ReputationMissionStartTimeS2CPacket;
...
public class LoginInEvent {
    ...
    public static void Login0(PlayerEvent.PlayerLoggedInEvent event) throws ParseException, IOException {
        ...
        if (!player.level().isClientSide) {
            ...
            if (!data.contains("version:3.25.0") && player.experienceLevel >= 140) {
                Compute.FormatMSGSend(player,Component.literal("补偿").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSakura),
                        Component.literal("你有待领取的补偿，输入/vmd compensate [ice/devil/taboo/moon/castle/purple]领取补偿！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA));
            }

            if (data.contains("version:3.23.0")) data.remove("version:3.23.0");
...
/*            if (Utils.IpLoginMap.containsKey(serverPlayer.getName().getString()) && Utils.IpLoginMap.get(serverPlayer.getName().getString()).equals(serverPlayer.getIpAddress())) {
            ...
            if (!data.contains(StringUtils.SkillPoint_Total)) data.putInt(StringUtils.SkillPoint_Total,player.experienceLevel);
            if (!data.contains(StringUtils.AbilityPoint_Total)) data.putInt(StringUtils.AbilityPoint_Total,player.experienceLevel);
            ...
            if (!data.contains(StringUtils.WeeklyRefreshWeekNum)) {
                ...
                Compute.FormatMSGSend(player,Component.literal("日常").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFlexible),
                        Component.literal(""));
            }
            else {
...
/*            if (data.contains("SignIn")) {
            ...
            if(!data.contains("FirstReward")) {
                ...
                Compute.FormatBroad(player.level(),Component.literal("维瑞阿契").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE),
                        Component.literal("欢迎新地质学家").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GOLD).
                                append(player.getDisplayName()).
                                append(Component.literal("的到来！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GOLD)));
                Compute.FormatMSGSend(player,Component.literal("欢迎").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA),
                        Component.literal("欢迎新人！新手教程请查看群文件内玩家编写的教程或查阅游戏内的帕秋莉手册(维瑞阿契wiki),游玩过程有任何建议或问题欢迎在群里@群主或管理员！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                Compute.FormatMSGSend(player,Component.literal("欢迎").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA),
                        Component.literal("您可以先打开身份卡，点击物品图鉴，浏览由制作者编写的各种装备，找到心仪的装备制作吧！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GOLD));
            }
...
/*            ModNetworking.sendToClient(new MissionResetS2CPacket(),serverPlayer);
            ...
            if (MissionNum < 11) {
                Compute.FormatMSGSend(player,Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSpider),
                        Component.literal("您有尚未完成的任务。[按大小写切换键呼出/隐藏任务纸条，你也可以前往按键绑定自行调整。]").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                Compute.FormatMSGSend(player,Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSpider),
                        Component.literal("1.在任务纸条呼出时，你可以使用准星标定来辅助你寻找任务目的地。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                Compute.FormatMSGSend(player,Component.literal("任务").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSpider),
                        Component.literal("2.在你进行游览任务时，你将不会受到伤害，但同时，你也无法造成伤害。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            ...
            Utils.recallList.forEach(recall -> {
                if (recall != null && recall.RecallPlayer != null) {
                    if (recall.RecallPlayer.equals(serverPlayer)) {
                        Compute.FormatBroad(player.level(),Component.literal("回忆").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfEnd),
                                Component.literal("").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                        append(Utils.SpiderRecall.playerName).
                                        append(Component.literal("尝试回想其在").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                        append(Component.literal(recall.zoneName).withStyle(ChatFormatting.RESET).withStyle(recall.style)).
            ...
            if (Utils.playerTeamMap.containsKey(player)) {
                ...
                if (playerTeam.getTeamLeader().equals(player)) {
                    ...
                    teamLeader.sendSystemMessage(Component.literal("").append(player.getDisplayName()).
                            append(Component.literal("离开了你的队伍。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
                    player.sendSystemMessage(Component.literal("您已退出").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
