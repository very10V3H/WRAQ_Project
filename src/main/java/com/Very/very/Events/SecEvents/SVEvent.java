...
public class SVEvent {
    @SubscribeEvent
    public static void SV(TickEvent.PlayerTickEvent event) throws ParseException {
        if(event.side.isServer() && event.phase == TickEvent.Phase.START) {
            ...
            if(TmpNum % 20 == 0) // 隐藏副本：火山
            {
                        ...
                        player.getZ() > 989 && player.getZ() < 991 && player.getY() == -54)
                {
                    ...
                    if(data.contains("KillCountOfVolcanoBlazw") && data.getInt("KillCountOfVolcanoBlazw") > 1000){
                            ...
                            else
                            {
                                if (Utils.SVController == 0) {
                                    ...
                                    Compute.FormatBroad(player.level(),Component.literal("隐藏副本").withStyle(ChatFormatting.RESET).
                                            withStyle(ChatFormatting.YELLOW),Component.literal(player.getName().getString()+"已激活火山隐藏副本！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                    ...
                    else
                    {
                        ...
                        if(data.contains("KillCountOfVolcanoBlazw") && data.getInt("KillCountOfVolcanoBlazw") < 1000){
                            Compute.FormatMSGSend(player,Component.literal("隐藏副本").withStyle(ChatFormatting.RESET).
                                    withStyle(ChatFormatting.YELLOW),Component.literal("火山烈焰").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.YELLOW).
                                    append(Component.literal("击杀数: ").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                            append(Component.literal("("+data.getInt("KillCountOfVolcanoBlazw")+"/"+1000+")"))));
                        }
                    }
                }
                if(data.contains("SVTIME"))
                {
                    ...
                    if(data.getInt("SVTIME") == 100) Compute.FormatMSGSend(player,Component.literal("隐藏副本").withStyle(ChatFormatting.RESET).
                            withStyle(ChatFormatting.YELLOW),Component.literal("还剩下100s！"));
                    if(data.getInt("SVTIME") == 80) Compute.FormatMSGSend(player,Component.literal("隐藏副本").withStyle(ChatFormatting.RESET).
                            withStyle(ChatFormatting.YELLOW),Component.literal("还剩下80s！"));
                    if(data.getInt("SVTIME") == 60) Compute.FormatMSGSend(player,Component.literal("隐藏副本").withStyle(ChatFormatting.RESET).
                            withStyle(ChatFormatting.YELLOW),Component.literal("还剩下60s！"));
                    if(data.getInt("SVTIME") == 40) Compute.FormatMSGSend(player,Component.literal("隐藏副本").withStyle(ChatFormatting.RESET).
                            withStyle(ChatFormatting.YELLOW),Component.literal("还剩下40s！"));
                    if(data.getInt("SVTIME") == 20) Compute.FormatMSGSend(player,Component.literal("隐藏副本").withStyle(ChatFormatting.RESET).
                    ...
                    if(data.getInt("SVTIME") == 0) {
                        ...
                        Compute.FormatBroad(player.level(),Component.literal("隐藏副本").withStyle(ChatFormatting.RESET).
                                withStyle(ChatFormatting.YELLOW),Component.literal(player.getName().getString()+"在火山隐藏副本激活期间，共击杀了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                append(Component.literal(""+data.getInt("KillCountOfSVC")).withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.YELLOW)).
                                append(Component.literal("只").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                append(Component.literal("隐秘熔岩能源").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.YELLOW)));
                        data.putInt("KillCountOfSVC",0);
                    }
                }
            }
        }
