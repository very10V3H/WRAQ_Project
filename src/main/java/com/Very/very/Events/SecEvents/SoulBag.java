...
public class SoulBag extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag flag) {
        ...
        if (Count == 0) Count = 64;
        components.add(Component.literal("内含：").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA).
                append(item.getDefaultInstance().getDisplayName()).
                append(Component.literal(" * " + Count).withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
