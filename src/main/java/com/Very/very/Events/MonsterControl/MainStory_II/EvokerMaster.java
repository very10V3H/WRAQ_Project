                ...
                for (Evoker evoker : Utils.EvokerMaster) {
                    if (evoker != null && evoker.isAlive()) {
                        ...
                        for (Player player : playerList) {
                            Compute.PlayerManaAddOrCost(player, (int) ((100 + Compute.PlayerAttributes.PlayerMaxMana(player)) * -0.5));
                            Compute.FormatMSGSend(player,Component.literal("唤魔大师").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.LIGHT_PURPLE),
                                    Component.literal("唤魔大师偷取了你的").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                            append(Compute.AttributeDescription.MaxMana("50%")));
                            Compute.Damage.ManaDamageToPlayer(evoker,player,((100 + Compute.PlayerAttributes.PlayerMaxMana(player)) * 0.5));
