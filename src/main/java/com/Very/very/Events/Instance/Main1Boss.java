            ...
            if (Utils.instanceList.get(2).isChallenge()) {
                    ...
                    if (instanceTick % 20 == 0) {
                        if (instanceTick == 200) {
                            ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                    new ClientboundSetTitleTextPacket(Component.literal("Main1Boss挑战").withStyle(Utils.styleOfSnow));
                ...
                if (instanceTick == 200) {
                    ...
                    Compute.SetMobCustomName(entity, ModItems.ArmorMain1Boss.get(),
                            Component.literal("四元方块").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSnow));
                ...
                if (instanceTick >= 200 && instance.getMobList().get(0) != null && Utils.instanceKillCount[2] == 1) {
                    ...
                    playerListGetByName.forEach(player -> {
                        ...
                        Compute.ExpPercentGetAndMSGSend(player,0.3 * difficultyEnhanceRate,Compute.PlayerExpUp(player),40);

                        ItemStack itemStack1 = Main1GemsGive(player,difficultyEnhanceRate);

                        Compute.FormatBroad(level, Component.literal("BOSS").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.DARK_RED),
                                Component.literal(player.getName().getString() + "击败了MainIBoss,").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
    ...
    public static ItemStack Main1GemsGive(Player player,int difficultyEnhanceRate) {
        ...
        switch (r.nextInt(4)) {
            case 0 -> {
                ...
                for (int j = 0; j < difficultyEnhanceRate; j++) {
