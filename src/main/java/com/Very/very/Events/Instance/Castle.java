...
public class Castle {
        ...
        if (!ZoneMobFlag.get(index) && DetectZoneMobClear(index) && !ZoneClearFlag.get(index)) {
            ...
            playerList.forEach(player -> {
                Compute.FormatMSGSend(player, Component.literal("暗黑城堡").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle),
                        Component.literal("第" + (index + 1) + "间怪物已清除！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                append(Component.literal("(" + ClearZoneNum() + "/" + "8)").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA)));
    ...
    public static void SummonNorthMon(List<Mob> mobList, Vec3 pos, Level level) {
        ...
        mobList.forEach(mob -> {
            Compute.SetMobCustomName(mob, ModItems.MobArmorBlackCastleOneFloorManaHelmet.get(), Component.literal("暗黑城堡遗魂 - 术士").withStyle(Utils.styleOfCastle));
    ...
    public static void SummonSouthMon(List<Mob> mobList, Vec3 pos, Level level) {
        ...
        mobList.forEach(mob -> {
            Compute.SetMobCustomName(mob, ModItems.MobArmorBlackCastleOneFloorAttackHelmet.get(), Component.literal("暗黑城堡遗魂 - 骑士").withStyle(Utils.styleOfCastle));
    ...
    public static void BossMSGSend(List<Player> playerList) {
        ...
        ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                new ClientboundSetTitleTextPacket(Component.literal("敌方增援马上赶到").withStyle(Utils.styleOfCastle));
        ClientboundSetSubtitleTextPacket clientboundSetSubtitleTextPacket =
                new ClientboundSetSubtitleTextPacket(Component.literal("前往城堡大门接战！").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle));
        playerList.forEach(player -> {
    ...
    public static void SummonBoss(List<Player> playerList, Level level) {
        ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                new ClientboundSetTitleTextPacket(Component.literal("敌方增援已经赶到").withStyle(Utils.styleOfCastle));
        ClientboundSetSubtitleTextPacket clientboundSetSubtitleTextPacket =
                new ClientboundSetSubtitleTextPacket(Component.literal("前往城堡大门迎战！").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle));
        playerList.forEach(player -> {
            ServerPlayer serverPlayer = (ServerPlayer) player;
            serverPlayer.connection.send(clientboundSetTitleTextPacket);
            serverPlayer.connection.send(clientboundSetSubtitleTextPacket);
        ...
        for (int i = 0 ; i < 4 ; i ++) {
            ...
            LightningSummon(mob);
            Compute.SetMobCustomName(mob, ModItems.MobArmorBlackCastleOneFloorManaHelmet.get(), Component.literal("暗黑城堡禁军 - 护卫").withStyle(Utils.styleOfCastle));
        ...
        for (int i = 4 ; i < 6; i ++) {
            Mob mob = BossList.get(i);
            LightningSummon(mob);
            Compute.SetMobCustomName(mob, ModItems.MobArmorBlackCastleOneFloorAttackHelmet.get(), Component.literal("暗黑城堡禁军 - 统领").withStyle(Utils.styleOfCastle));
            mob.setItemSlot(EquipmentSlot.HEAD, ModItems.MobArmorBlackCastleOneFloorAttackHelmet.get().getDefaultInstance());
            mob.setItemSlot(EquipmentSlot.CHEST, ModItems.MobArmorBlackCastleOneFloorChest.get().getDefaultInstance());
            mob.setItemSlot(EquipmentSlot.LEGS, ModItems.MobArmorBlackCastleOneFloorLeggings.get().getDefaultInstance());
            mob.setItemSlot(EquipmentSlot.FEET, ModItems.MobArmorBlackCastleOneFloorBoots.get().getDefaultInstance());
            ...
            mob.getAttribute(Attributes.MAX_HEALTH).setBaseValue(5 * Math.pow(10,8));
            mob.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(0.4D);
            mob.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(10000);
            mob.setHealth(mob.getMaxHealth());
            mob.moveTo(BossPos.x + random.nextInt(6) - 3, BossPos.y, BossPos.z + random.nextInt(6) - 3);
            ...
            if (Utils.instanceList.get(InstanceIndex).isChallenge()) {
                ...
                if (instanceTick <= startTick) {
                    if (instanceTick % 20 == 0) {
                        if (instanceTick == startTick) {
                            ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                    new ClientboundSetTitleTextPacket(Component.literal("暗黑城堡 - 1层").withStyle(Utils.styleOfCastle));
                            ClientboundSetSubtitleTextPacket clientboundSetSubtitleTextPacket =
                        ...
                        if (instanceTick < startTick) {
                            ...
                            list.forEach(mob -> {
                                if (!mob.getItemBySlot(EquipmentSlot.HEAD).is(ModItems.MobArmorCastleKnightHelmet.get())) mob.remove(Entity.RemovalReason.KILLED);
                            });
                        }
    ...
    public static void Reward(List<Player> playerListGetByName, Instance instance, int playerNum,
        ...
        playerListGetByName.forEach(player -> {
            ...
            if (CauseDamagePlayer.contains(player)) {
                try {
