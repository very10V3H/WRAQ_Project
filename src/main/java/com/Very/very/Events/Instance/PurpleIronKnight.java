...
public class PurpleIronKnight {
    ...
    public static void PurpleIronKnightEvent(TickEvent.LevelTickEvent event) {
        if (event.side.isServer() && event.phase.equals(TickEvent.Phase.START)
            ...
            double MaxHealth = 5 * Math.pow(10,9); // 生命值
            if (Utils.instanceList.get(InstanceIndex).isChallenge()) {
                    ...
                    if (instanceTick % 20 == 0) {
                        if (instanceTick == 200) {
                            ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                    new ClientboundSetTitleTextPacket(Component.literal("紫晶骑士").withStyle(Utils.styleOfPurpleIron));
                ...
                if (instanceTick == 200) {
                    ...
                    Compute.SetMobCustomName(entity, ModItems.MobArmorPurpleIronKnightHelmet.get(),
                            Component.literal("紫晶骑士").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfPurpleIron));
                ...
                if (instanceTick > 200 && instance.getMobList().get(0) != null) {
                    ...
                    if (instanceTick % 100 == 0) for (int i = 0 ; i < 10 ; i ++) LineAttack(mob,instance.getTeleportPosition(),32,10000 * difficultyEnhanceRate,1);
                    if (instanceTick % 100 == 50) {
                        for (int i = 0 ; i < 10 ; i ++) CircleAttack(mob,instance.getTeleportPosition(),4,10000 * difficultyEnhanceRate,1);
                        playerListGetByName.forEach(player -> {
                    ...
                    playerListGetByName.forEach(player -> {
                        if (PlayerRewardGetJudge(player)) {
                            ...
                            if (random.nextDouble() <= 0.01) {
                                ...
                                Compute.FormatBroad(level,Component.literal("紫晶骑士").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfPurpleIron),
    ...
    public static boolean PlayerRewardGetJudge(Player player) {
        ...
        if (!data.contains(RewardGetLimit) || data.getInt(RewardGetLimit) < RewardGetTimes) {
            ...
            Compute.FormatMSGSend(player,Component.literal("紫晶").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfPurpleIron),
                    Component.literal("当日可用奖励获取次数: ").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                            append(Component.literal(data.getInt(RewardGetLimit) + "/" + RewardGetTimes).withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfPurpleIron)));
            return true;
        }
        Compute.FormatMSGSend(player,Component.literal("紫晶").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfPurpleIron),
                Component.literal("当日可用奖励获取次数已耗尽。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
        return false;
    }
