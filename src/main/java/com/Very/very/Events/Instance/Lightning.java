...
public class Lightning {
                    ...
                    if (instanceTick % 20 == 0) {
                        if (instanceTick == 200) {
                            ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                    new ClientboundSetTitleTextPacket(Component.literal("唤雷岛挑战").withStyle(Utils.styleOfLightingIsland));
                ...
                if (instanceTick > 200) {
                    if (instanceTick % 200 == 0) {
                        playerListGetByName.forEach(player -> {
                            Compute.FormatMSGSend(player,Component.literal("唤雷").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfLightingIsland),
                                    Component.literal("距离挑战结束，还有").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                            append(Component.literal(  "" + (1400 - instanceTick) / 20 + "秒").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
                        });
                    }
                }
                ...
                if (instanceTick >= 1400) {
                    ...
                    Compute.FormatBroad(level,Component.literal("副本").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                            Component.literal("队伍:").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                    ...
                                    append(difficulty).
                                    append(Component.literal("共击杀了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                    append(Component.literal("" +Utils.instanceKillCount[1]).withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfLightingIsland)).
                                    append(Component.literal("只唤雷守卫。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
                    playerListGetByName.forEach(player -> {
    ...
    public static void MobAttributeSet(Mob mob,int difficultyRate,int playerNum) {
       ...
       mob.getAttribute(Attributes.MAX_HEALTH).setBaseValue(20000.0D * difficultyRate * (1 + (playerNum- 1) * 0.75));
       mob.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(60.0D * difficultyRate);
