...
public class CastleSecondFloor {
    ...
    public static double MaxHealth = 2 * Math.pow(10,9);
    ...
    public static void PlayerPickItemExDamage(Player player, Mob mob) {
        if (!mob.getItemBySlot(EquipmentSlot.HEAD).is(ModItems.MobArmorCastleKnightHelmet.get())) return ;
        if (playerPickedItemMap.containsKey(player) && playerPickedItemMap.get(player)) {
            ...
            Compute.FormatMSGSend(player,Component.literal("暗黑城堡").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle),
                    Component.literal("暗黑骑士还有" + shieldCount + "层顽盾!").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            ClientboundSoundPacket clientboundSoundPacket =
    ...
    public static double MobDamageImmune(Player player, Mob mob) {
        if (!mob.getItemBySlot(EquipmentSlot.HEAD).is(ModItems.MobArmorCastleKnightHelmet.get())) return 1;
        if (shieldCount > 0) {
            Compute.FormatMSGSend(player,Component.literal("暗黑城堡").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle),
                    Component.literal("暗黑骑士还有" + shieldCount + "层顽盾!").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            ...
            if (Utils.instanceList.get(InstanceIndex).isChallenge()) {
                    ...
                    if (instanceTick % 20 == 0) {
                        if (instanceTick == startTick) {
                            ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                    new ClientboundSetTitleTextPacket(Component.literal("暗黑城堡 - 2层").withStyle(Utils.styleOfCastle));
                ...
                if (instanceTick > startTick && instance.getMobList().get(0) != null) {
                    ...
                    if (instance.getMobList().get(0).isAlive() && (instanceTick - startTick) % 200 == 30) {
                        ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                new ClientboundSetTitleTextPacket(Component.literal("暗黑爆裂").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle));
                        ClientboundSetSubtitleTextPacket clientboundSetSubtitleTextPacket =
                                new ClientboundSetSubtitleTextPacket(Component.literal("请迅速远离其他玩家！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                        playerListGetByName.forEach(player -> {
                    ...
                    if (instance.getMobList().get(1).isAlive() && (instanceTick - startTick) % 200 == 130) {
                        ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                new ClientboundSetTitleTextPacket(Component.literal("暗黑聚能").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle));
                        ClientboundSetSubtitleTextPacket clientboundSetSubtitleTextPacket =
                                new ClientboundSetSubtitleTextPacket(Component.literal("请与其他玩家站在一起！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                        playerListGetByName.forEach(player -> {
                            ((ServerPlayer) player).connection.send(clientboundSetSubtitleTextPacket);
                            ((ServerPlayer) player).connection.send(clientboundSetTitleTextPacket);
                        });
                    ...
                    if (((instance.getMobList().get(0).isAlive() && !instance.getMobList().get(1).isAlive())
                        ...
                        ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                new ClientboundSetTitleTextPacket(Component.literal("暗黑能量方块已生成！").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle));
                        ClientboundSetSubtitleTextPacket clientboundSetSubtitleTextPacket =
                                new ClientboundSetSubtitleTextPacket(Component.literal("获取蕴魔石，击碎骑士顽盾！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                        playerListGetByName.forEach(player -> {
                            ((ServerPlayer) player).connection.send(clientboundSetSubtitleTextPacket);
                            ((ServerPlayer) player).connection.send(clientboundSetTitleTextPacket);
                        });
                    ...
                    if (!passiveFlag && shieldCount == 0) {
                        shieldCount = -1;
                        ClientboundSetTitleTextPacket clientboundSetTitleTextPacket =
                                new ClientboundSetTitleTextPacket(Component.literal("骑士顽盾已破碎！").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle));
                        ClientboundSetSubtitleTextPacket clientboundSetSubtitleTextPacket =
                                new ClientboundSetSubtitleTextPacket(Component.literal("迅速击杀暗黑骑士！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                        playerListGetByName.forEach(player -> {
                            ((ServerPlayer) player).connection.send(clientboundSetSubtitleTextPacket);
                            ((ServerPlayer) player).connection.send(clientboundSetTitleTextPacket);
                        });
                    ...
                    if (!passiveFlag && shieldCount == -1 && instanceTick % 100 == 0) {
                        if (instance.getMobList().get(0).isAlive()) instance.getMobList().get(0).heal((float) (MaxHealth * 0.02));
                        if (instance.getMobList().get(1).isAlive()) instance.getMobList().get(1).heal((float) (MaxHealth * 0.02));
                    }
    ...
    public static void Reward(List<Player> playerListGetByName, Instance instance, int playerNum,
                ...
                if (boss2Damage.getPlayer() != null) {
                    ...
                    Compute.FormatMSGSend(player,Component.literal(index + ".").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                            Component.literal(" ").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                    ...
                                    append(Component.literal("  DMG:" + damage + "[" + String.format("%.2f",damage * 100 / (MaxHealth * 2)) + "%]").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
                    index.incrementAndGet();
                }
            });

            if (playerDamageCount.containsKey(player) && playerDamageCount.get(player) / (MaxHealth * 2) > 0.1) {
                try {
                ...
                if (random.nextDouble() <= 0.1 && difficultyEnhanceRate == 4) {
                    ...
                    Compute.FormatBroad(level,Component.literal("黑金").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle),
                            Component.literal("").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
        ...
        Compute.FormatBroad(level, Component.literal("副本").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                Component.literal("队伍:").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                        ...
                        append(Component.literal(String.format("%.2f", (instanceTick - startTick) * 0.05) + "s ").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfBloodMana)).
    ...
    public static void Summon(Mob mob, Instance instance, int index) {
        ...
        Compute.SetMobCustomName(mob, ModItems.MobArmorCastleKnightHelmet.get(),
                Component.literal("暗黑骑士").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle));
