...
public class DpsCommand implements Command<CommandSourceStack> {
    ...
    public static void Tick(Player player) {
        ...
        if (playerDpsStartTickMap.containsKey(player) && playerDpsStartTickMap.get(player) >= TickCount) {
            if ((playerDpsStartTickMap.get(player) - TickCount) % 20 == 0) {
                ...
                if (playerDpsStartTickMap.get(player) == TickCount) {
                    clientboundSetTitleTextPacket = new ClientboundSetTitleTextPacket(Component.literal("Dps计算已开始").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
        ...
        if (playerDpsTickMap.containsKey(player) && playerDpsTickMap.get(player) == TickCount) {
            ...
            Compute.FormatMSGSend(player,Component.literal("Dps").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                    Component.literal("在" + playerDpsSecondMap.get(player) + "s内的Dps值如下:").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            player.sendSystemMessage(Component.literal("  1.在持续时间内的总伤害: ").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                    append(Component.literal(FormatDamageCount(damageCount))));
            player.sendSystemMessage(Component.literal("  2.Dps(damage per second)每秒造成的伤害: ").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
