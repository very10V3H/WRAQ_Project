...
public class deleteCommand implements Command<CommandSourceStack> {
    ...
    public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (Utils.deleteCommandSecurity.containsKey(player) && Utils.deleteCommandSecurity.get(player) > TickCount) {
            ...
            Compute.FormatMSGSend(player, Component.literal("销毁").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                    Component.literal("请在3s内再次输入指令以进行销毁。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
        }
