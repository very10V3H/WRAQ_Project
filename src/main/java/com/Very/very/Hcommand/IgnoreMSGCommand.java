    ...
    public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (string.equalsIgnoreCase("ItemGet")) {
            ...
            if (!data.contains(Type) || !data.getBoolean(Type)) {
                data.putBoolean(Type, true);
                Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                        Component.literal("你已屏蔽所有物品获取信息。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));

            } else {
                data.putBoolean(Type, false);
                Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                        Component.literal("你重新开始接收物品获取信息。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            }
            flag = false;
        }
        if (string.equalsIgnoreCase("VB")) {
            ...
            if (!data.contains(Type) || !data.getBoolean(Type)) {
                data.putBoolean(Type, true);
                Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                        Component.literal("你已屏蔽所有VB信息。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));

            } else {
                data.putBoolean(Type, false);
                Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                        Component.literal("你重新开始接收VB信息。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            }
            flag = false;
        }
        if (string.equalsIgnoreCase("Mana")) {
            ...
            if (!data.contains(Type) || !data.getBoolean(Type)) {
                data.putBoolean(Type, true);
                Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                        Component.literal("你已屏蔽所有法力值信息。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));

            } else {
                data.putBoolean(Type, false);
                Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                        Component.literal("你重新开始接收法力值信息。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            }
            flag = false;
        }
        if (string.equalsIgnoreCase("Instance")) {
            ...
            if (!data.contains(Type) || !data.getBoolean(Type)) {
                data.putBoolean(Type, true);
                Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                        Component.literal("你已屏蔽所有副本信息。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));

            } else {
                data.putBoolean(Type, false);
                Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                        Component.literal("你重新开始接收副本信息。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            }
            flag = false;
        }
        if (flag) Compute.FormatMSGSend(player, Component.literal("屏蔽").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED),
                Component.literal("当前支持的屏蔽类型为战斗(Fight)、符石(Rune)、经验(Exp)、快捷使用(QuickUse)、物品获取(ItemGet)、金钱(VB)、法力值(Mana)、副本(Instance)。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
