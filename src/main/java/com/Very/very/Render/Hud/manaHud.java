...
import com.Very.very.Events.ClientEvents.ClientTickEvent;
...
/*            for(int i = 0; i < 9; i++) {
            ...
            if (mc.player.isShiftKeyDown()) {
                guiGraphics.drawCenteredString(fontRenderer,Component.literal(  "Ps:"+ ClientUtils.currentPsValue + "/100").withStyle(ChatFormatting.YELLOW),x + 67,y - 54,0);

            }
            if (!mc.player.isShiftKeyDown()) {
                guiGraphics.blit(MANA_PANEL,x + 75,y - 47,0,0,64,64,64,64);
                int MaxMana = ClientManaData.getMaxMana();
                int CurrentMana = ClientManaData.getCurrentMana();
                int Rate = (int) Math.floor (20 * ( (double) CurrentMana /  MaxMana));
                for (int i = 0; i <= 20; i++) {
                    if (Rate >= i) {
                ...
                guiGraphics.drawCenteredString(fontRenderer,Component.literal("" + CurrentMana).withStyle(Utils.styleOfMana),x + 107,y - 38,0);
                ...
                if (MaxSwift != 0) {
                    for (int i = 0; i < 3; i ++) {
                        if (CurrentSwift >= (33 * (i + 1))) guiGraphics.blit(Swiftness_Full,x + 120,y - 17 - i * 8,0,0,20,20,20,20);
