...
import com.Very.very.NetWorking.Customized.LiuLiXianActiveC2SPacket;
import com.Very.very.NetWorking.Customized.LiuLiXianSceptreC2SPacket;
import com.Very.very.NetWorking.Customized.YuanShiRenC2SPacket;
import com.Very.very.NetWorking.ModNetworking;
import com.Very.very.NetWorking.Packets.AnimationPackets.RollingAnimationRequestC2SPacket;
                ...
                if (KeyBoradInput.NavigateSet.consumeClick()) {
                    if (ClientUtils.Mission) {
                        if (ClientUtils.NavigateIndex == -1) {
                            ...
                            Minecraft.getInstance().player.sendSystemMessage(Component.literal("[系统]").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY).
                                    append(Component.literal("已启用目的地准星定点，请尝试移动准星直到无法看见红色粒子，位置即为目的地方向。[默认左Alt开启或关闭]").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
                        }
                        else {
                            ClientUtils.NavigateIndex = -1;
                            Minecraft.getInstance().player.sendSystemMessage(Component.literal("[系统]").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY).
                                    append(Component.literal("已关闭目的地准星定点。[默认左Alt开启或关闭]").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));

                        }
                    }
                    else {
                        Minecraft.getInstance().player.sendSystemMessage(Component.literal("[系统]").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY).
                                append(Component.literal("似乎没有任务坐标用于准星定点。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
