...
public class Parkour {
        ...
        if (TickCount % 5 == 2) {
            ...
            if (point == currentPoint + 1) {
                ...
                Compute.FormatMSGSend(player, Component.literal("跑酷").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFlexible),
                        Component.literal("已经保存记录点！(" + point + ")").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
                ...
                if (point % 5 == 0 && lastRewardPoint != point) {
                    ...
                    Compute.FormatBroad(player.level(),Component.literal("跑酷").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFlexible),
                            Component.literal("").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                    append(player.getDisplayName()).
                                    append(Component.literal(" 完成了一个跑酷阶段!  ").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFlexible)).
                                    append(Component.literal("(" + GetRewardTimes(player) + ")").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFlexible)));
            ...
            if (PlayerIsInParkourRange(player) && player.isFallFlying()) {
                Compute.RespawnPlayer(player);
                Compute.FormatMSGSend(player,Component.literal("跑酷").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFlexible),
                        Component.literal("在跑酷区域不能使用鞘翅哦！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
            }
    ...
    public static void TpToCurrentParkourPointPos(Player player) {
        ...
        Compute.FormatMSGSend(player,Component.literal("跑酷").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFlexible),
                Component.literal("你已回到跑酷记录点/起点，继续你的跑酷吧！").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
