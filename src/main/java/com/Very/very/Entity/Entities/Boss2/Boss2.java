package com.Very.very.Entity.Entities.Boss2;

import com.Very.very.NetWorking.ModNetworking;
import com.Very.very.NetWorking.Packets.AnimationPackets.Boss2AnimationStartS2CPacket;
            ...
            if (AttackTick == 0) {
                switch (AttackMode) {
                    ...
                    case 3 -> {
                        List<Player> playerList = this.level().getEntitiesOfClass(Player.class, AABB.ofSize(this.pick(3,0,true).getLocation(),10,10,10));
                        playerList.forEach(player -> {
                            if (player.onGround()) Compute.Damage.AttackDamageToPlayer(this,player,200 * (1 + Utils.Boss2DeadTimes));
                        });
                    }
                }
            }
