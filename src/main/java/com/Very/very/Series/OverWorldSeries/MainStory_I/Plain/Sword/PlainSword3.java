package com.Very.very.Series.OverWorldSeries.MainStory_I.Plain.Sword;
...
public class PlainSword3 extends SwordItem{
    private final float BaseDamage = 30.0F;
    private final float BreakDefence = 0.0F;
    private final float CriticalHitRate = 0.25F;
    private final float CHitDamage = 0.9F;
    private final float HealSteal = 0.05F;
    private final float SpeedUp = 0.3F;
    public PlainSword3(Tier tier, int num1, float num2){
    ...
    {
        Compute.ForgingHoverName(stack,Component.literal("平原短匕-III").withStyle(ChatFormatting.GREEN).withStyle(ChatFormatting.BOLD));
        ...
        Compute.DescriptionActive(components,Component.literal("平原洗礼-III").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GREEN));
        components.add(Component.literal("右键恢复").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.Health("20%")));
        Compute.CoolDownTimeDescription(components,20);
        Compute.EmojiDescriptionManaCost(components,20);
        Compute.DescriptionDash(components,ChatFormatting.WHITE,ChatFormatting.GREEN,ChatFormatting.WHITE);
        components.add(Component.literal("Plain-Knife-III").withStyle(ChatFormatting.GREEN).withStyle(ChatFormatting.ITALIC));
