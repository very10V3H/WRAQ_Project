...
import java.util.List;
 ...
 public class BlackForestSwordForgingDrawing extends Item {
     private final Item Material1 = Moditems.BlackForestRune.get();
     ...
     public BlackForestSwordForgingDrawing(Properties p_41383_) {
         super(p_41383_);
     }
     ...
     public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
         ...
         components.add(Component.literal("·制作:灵魂收割者*1").withStyle(Utils.styleOfBlackForest));
         ...
         components.add(Component.literal("ForgingDrawing-BlackForest").withStyle(Utils.styleOfBlackForest).withStyle(ChatFormatting.ITALIC));
     ...
     public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
         if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND))
         {
             ...
             if(ContainMaterial)
             {
                 ...
                 ItemStack itemStack = Moditems.BlackForestSword0.get().getDefaultInstance();
                 itemStack.getOrCreateTagElement(Utils.MOD_ID).putInt("Number", ConfigTest.BlackForestSword.get());
                 Compute.FormatBroad(level,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                         Component.literal(player.getName().getString()+"成功锻造了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).append(itemStack.getDisplayName()).
                                 append(Component.literal("这是维瑞阿契第"+ConfigTest.BlackForestSword.get()+"件").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                 append(Component.literal("灵魂收割者").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfBlackForest)));
                 ConfigTest.BlackForestSword.set(ConfigTest.BlackForestSword.get()+1);
