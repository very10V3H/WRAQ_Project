...
public class PlainSwrod1 extends SwordItem{
    private final float BaseDamage = 15.0F;
    private final float BreakDefence = 0.0F;
    private final float CriticalHitRate = 0.25F;
    private final float CHitDamage = 0.6F;
    private final float HealSteal = 0.05F;
    private final float SpeedUp = 0.15F;
    public PlainSwrod1(Tier tier, int num1, float num2){
    ...
    {
        Compute.ForgingHoverName(stack,Component.literal("平原短匕-I").withStyle(ChatFormatting.GREEN).withStyle(ChatFormatting.BOLD));
        ...
        Compute.DescriptionActive(components,Component.literal("平原洗礼-I").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GREEN));
        components.add(Component.literal("右键恢复").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.Health("12%")));
        Compute.CoolDownTimeDescription(components,20);
        Compute.EmojiDescriptionManaCost(components,20);
        Compute.DescriptionDash(components,ChatFormatting.WHITE,ChatFormatting.GREEN,ChatFormatting.WHITE);
        components.add(Component.literal("Plain-Knife-I").withStyle(ChatFormatting.GREEN).withStyle(ChatFormatting.ITALIC));
