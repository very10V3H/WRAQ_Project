...
public class LALForgingDrawing extends Item {

    public LALForgingDrawing(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        components.add(Component.literal("·制作:唤雷之源*1").withStyle(Utils.styleOfLightingIsland));
        components.add(Component.literal("·材料需求:").withStyle(ChatFormatting.AQUA));
        components.add(Compute.MaterialRequirement(1,Component.literal("唤雷之芽").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfLightingIsland),14));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND))
        {
            ...
            boolean ContainMaterial = Compute.ItemStackCheck(inventory, Moditems.LightningRune.get(),14)
                    && Compute.ItemStackCheck(inventory, Moditems.GoldCoin.get(),64)
                    && Compute.ItemStackCheck(inventory, Moditems.MineSoul.get(),128);
            if(ContainMaterial)
            {
                Compute.ItemStackRemove(inventory,Moditems.LightningRune.get(),14);
                ...
                ItemStack itemStack = Moditems.IslandArmorLeggings.get().getDefaultInstance();
                itemStack.getOrCreateTagElement(Utils.MOD_ID).putInt("Number", ConfigTest.ForgingLightningArmorLeggings.get());
                Compute.FormatBroad(level,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                        Component.literal(player.getName().getString()+"成功锻造了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).append(itemStack.getDisplayName()).
                                append(Component.literal("这是维瑞阿契第"+ConfigTest.ForgingLightningArmorLeggings.get()+"件").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                append(Component.literal("唤雷之源").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfLightingIsland)));
                ConfigTest.ForgingLightningArmorLeggings.set(ConfigTest.ForgingLightningArmorLeggings.get()+1);
            ...
            else
            {
                ...
                if(Compute.ItemStackCount(inventory,Moditems.LightningRune.get()) < 14)
                {
                    Compute.FormatMSGSend(player,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                            Component.literal("缺少:").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                    ...
                                    append(Component.literal("*"+(14-Compute.ItemStackCount(inventory,Moditems.LightningRune.get())))));
                }
