package com.Very.very.Series.OverWorldSeries.MainStory_II.Sky.BossItems;
...
public class SkyBoss {
    public static Style chatFormatting = Utils.styleOfSky;
            ...
            if (!level.isClientSide) {
                ...
                Compute.FormatMSGSend(player,Component.literal("次元能量").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfEntropy),
                        Component.literal("你的").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                append(Component.literal(Name + "次元能量").withStyle(ChatFormatting.RESET).withStyle(chatFormatting)).
                                append(Component.literal("得到了提升。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                append(Component.literal("[" + data.getInt(Entropy) + "]").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY)));
            ...
            if (!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
                ...
                for(int i = 0; i < 8; i ++) {
                    if (random.nextDouble(1) < RewardsRate[i] * (1 + Compute.PlayerLuckyUp(player))) {
                        ...
                        Compute.FormatBroad(level,Component.literal("次元").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfEntropy),
                                Component.literal("玩家").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                        append(player.getDisplayName()).
                                        append(Component.literal("通过次元口袋获得了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                        append(Rewards[i].getDefaultInstance().getDisplayName()));
                    }
                }
                try {
    ...
    public static class SkyBossBow extends PickaxeItem {
        ...
        public void appendHoverText(ItemStack stack, @javax.annotation.Nullable Level level, List<Component> components, TooltipFlag flag)
        {
            ...
            Compute.ForgingHoverName(stack,Component.literal("<次元级>").withStyle(Utils.styleOfEntropy).append(Component.literal("寰宇之耀").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSky).withStyle(ChatFormatting.BOLD)));
            ...
            components.add(Component.literal(" - ").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY).
                    append(Component.literal("天空维度展开: ").withStyle(ChatFormatting.RESET).withStyle(MainStyle)).
                    append(Component.literal("天空制造者释放制造天空次元的能量，使持有者具有凌驾天空的力量。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
            components.add(Component.literal("∰1.使持有者至多获得").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                    append(Compute.AttributeDescription.CritDamage("320%")));

            Compute.DescriptionPassive(components,Component.literal("鹰隼之速").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.AQUA));
...
/*        @Override
        ...
        @Override
        public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
            Compute.BowAttack(player,Utils.BowNumMap.get(StringUtils.BowNameString.SkyBossBow));
            return super.use(level, player, interactionHand);
        }
