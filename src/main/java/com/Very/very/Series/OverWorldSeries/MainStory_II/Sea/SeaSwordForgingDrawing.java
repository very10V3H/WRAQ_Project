...
public class SeaSwordForgingDrawing extends Item {
    private final Item Material1 = Moditems.SeaRune.get();
    ...
    public SeaSwordForgingDrawing(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        components.add(Component.literal("·制作:灵魂救赎者*1").withStyle(Utils.styleOfSea));
        ...
        components.add(Component.literal("ForgingDrawing-Sea").withStyle(Utils.styleOfSea).withStyle(ChatFormatting.ITALIC));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND))
        {
            ...
            if(ContainMaterial)
            {
                ...
                ItemStack itemStack = Moditems.SeaSword0.get().getDefaultInstance();
                itemStack.getOrCreateTagElement(Utils.MOD_ID).putInt("Number", ConfigTest.SeaSword.get());
                Compute.FormatBroad(level,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                        Component.literal(player.getName().getString()+"成功锻造了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).append(itemStack.getDisplayName()).
                                append(Component.literal("这是维瑞阿契第"+ConfigTest.SeaSword.get()+"件").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                append(Component.literal("灵魂救赎者").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSea)));
                ConfigTest.SeaSword.set(ConfigTest.SeaSword.get()+1);
