...
public class ShipBow extends PickaxeItem {
    private final double BaseDamage = 280.0d;
    private final double DefencePenetration0 = 2200;
    private final double CriticalHitRate = 0.25;
    private final double CHitDamage = 3.6;
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> components, TooltipFlag flag)
    {
        ...
        Compute.DescriptionPassive(components,Component.literal("领航").withStyle(ChatFormatting.RESET).withStyle(style));
        components.add(Component.literal(" 根据周围玩家数量，为你提供").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.DefencePenetration("50-200")));
        components.add(Component.literal(" - 提供的护甲数额随周围玩家数量增长。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY).withStyle(ChatFormatting.ITALIC));
        Compute.DescriptionDash(components,ChatFormatting.WHITE,style,ChatFormatting.WHITE);
...
/*    @Override
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        Compute.BowAttack(player,Utils.BowNumMap.get(StringUtils.BowNameString.ShipBow));
