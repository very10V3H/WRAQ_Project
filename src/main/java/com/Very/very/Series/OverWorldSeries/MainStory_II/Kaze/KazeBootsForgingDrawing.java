...
import java.util.List;
 ...
 public class KazeBootsForgingDrawing extends Item {
     private final Item Material1 = Moditems.KazeRune.get();
     ...
     public KazeBootsForgingDrawing(Properties p_41383_) {
         super(p_41383_);
     }
     ...
     public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
         ...
         components.add(Component.literal("·制作:翠绿足具*1").withStyle(Utils.styleOfKaze));
         ...
         components.add(Component.literal("ForgingDrawing-Kaze").withStyle(Utils.styleOfKaze).withStyle(ChatFormatting.ITALIC));
     ...
     public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
         if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND))
         {
             ...
             if(ContainMaterial)
             {
                 ...
                 ItemStack itemStack = Moditems.KazeBoots.get().getDefaultInstance();
                 itemStack.getOrCreateTagElement(Utils.MOD_ID).putInt("Number", ConfigTest.KazeBoots.get());
                 Compute.FormatBroad(level,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                         Component.literal(player.getName().getString()+"成功锻造了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).append(itemStack.getDisplayName()).
                                 append(Component.literal("这是维瑞阿契第"+ConfigTest.KazeBoots.get()+"件").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                 append(Component.literal("翠绿足具").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze)));
                 ConfigTest.KazeBoots.set(ConfigTest.KazeBoots.get()+1);
