...
public class SeaBow extends PickaxeItem {
    ...
    private final double CHitDamage = 1.0d;
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> components, TooltipFlag flag)
    {
        ...
        Compute.DescriptionPassive(components,Component.literal("破碎晶石").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSea));
        components.add(Component.literal("箭矢命中目标后，会对目标周围所有单位造成").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                ...
                append(Component.literal("包括目标本身").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY)));
        components.add(Component.literal("若此真实伤害使目标死亡，则获得持续15s的").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Component.literal("等同于此伤害的护盾值").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
        Compute.DescriptionDash(components,ChatFormatting.WHITE,Utils.styleOfSea,ChatFormatting.WHITE);
...
/*    @Override
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        Compute.BowAttack(player,Utils.BowNumMap.get(StringUtils.BowNameString.SeaBow));
