...
public class SnowArmorHelmet extends ArmorItem {
    private final double MaxHealth = 100;
    private final double Defence = 40;
    private static final Style style = Utils.styleOfSnow;
    public SnowArmorHelmet(ItemMaterial Material, Type Slots)
    {
    ...
    {
        Compute.ForgingHoverName(stack,Component.literal("玉山头盔").withStyle(style).withStyle(ChatFormatting.BOLD));
        components.add(Component.literal("防具                   ").withStyle(ChatFormatting.GRAY).append(Component.literal("头盔").withStyle(ChatFormatting.RESET).withStyle(style)));
