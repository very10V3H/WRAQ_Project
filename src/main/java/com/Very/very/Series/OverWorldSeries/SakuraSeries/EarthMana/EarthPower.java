package com.Very.very.Series.OverWorldSeries.SakuraSeries.EarthMana;
...
public class EarthPower extends Item {
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag flag) {
        ...
        Compute.DescriptionActive(components,Component.literal("汲蕴").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfBloodMana));
        components.add(Component.literal(" 依据").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Component.literal("周边环境的物质").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfMoon)).
                append(Component.literal("提供不同的效果:").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));

        components.add(Component.literal(" 平原:").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfPlain).
                append(Component.literal(" 降低怪物造成的伤害20%").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
        components.add(Component.literal("    治疗附近所有玩家").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
        ...
        components.add(Component.literal(" 森林:").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfForest).
                append(Component.literal(" 对周围怪物造成减速效果").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
        components.add(Component.literal("    提升周围玩家的").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
        ...
        components.add(Component.literal(" 湖泊:").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfLake).
                append(Component.literal(" 削减周围怪物").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                append(Compute.AttributeDescription.ManaDefence("40%")));
        components.add(Component.literal("    使周围玩家获得").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.CoolDown("30")));

        components.add(Component.literal(" 火山:").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfVolcano).
                append(Component.literal(" 额外造成一次").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
        ...
        if (ClientUtils.EarthPowerType != -1) {
            components.add(Component.literal(" - 当前所处环境中的主导物质类型为: ").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfBloodMana).
                    append(Component.literal(elements[ClientUtils.EarthPowerType]).withStyle(ChatFormatting.RESET).withStyle(styles[ClientUtils.EarthPowerType])));
        }
            ...
            for (int j = 0 ; j < radius * 2 ; j ++) {
                for (int k = 0 ; k < radius ; k ++) {
                    BlockPos blockPos = new BlockPos(player.getBlockX() + i - radius,
                            player.getBlockY() + j - radius,
                            player.getBlockZ() + k - radius / 2);
                    if (BlockEvent.LeafMap.containsKey(player.level().getBlockState(blockPos).getBlock())
                            || BlockEvent.WoodMap.containsKey(player.level().getBlockState(blockPos).getBlock()))
                        count ++;
                }
            }
        }
    ...
    public static int ElementType(Player player) {
        if (CalculatePlainElement(player) > Math.max(Math.max(CalculateForestElement(player),CalculateLakeElement(player)),CalculateVolcanoElement(player))) return 0;
        if (CalculateForestElement(player) > Math.max(Math.max(CalculatePlainElement(player),CalculateLakeElement(player)),CalculateVolcanoElement(player))) return 1;
        if (CalculateLakeElement(player) > Math.max(Math.max(CalculatePlainElement(player),CalculateForestElement(player)),CalculateVolcanoElement(player))) return 2;
        return 3;
    }
}
