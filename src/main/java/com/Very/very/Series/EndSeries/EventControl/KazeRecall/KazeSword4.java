...
public class KazeSword4 extends SwordItem{
    private final float BaseDamage = 120.0F;
    private final float BreakDefence = 0.40F;
    private final float CriticalHitRate = 0.30F;
    private final float CHitDamage = 1.50F;
    private final float HealSteal = 0.08F;
    private final float SpeedUp = 0.8F;
    ...
    {
        Compute.ForgingHoverName(stack,Component.literal("<维瑞级>").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GOLD).append(Component.literal("<狂风能量激化>").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfVolcano)).append(Component.literal("灵风").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfKaze).withStyle(ChatFormatting.BOLD)));
        ...
        components.add(Component.literal("冷却时间: 0.7s").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfVolcano).
        ...
        components.add(Component.literal("若周围有单位位于空中，则对位于空中的所有单位").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
        components.add(Component.literal("造成").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.AttackDamage("1000%")).
                append(Component.literal("的物理伤害并将其强制牵引至地面。").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
        Compute.CoolDownTimeDescription(components,3);
        Compute.EmojiDescriptionManaCost(components,15);
        components.add(Component.literal(" "));
        components.add(Component.literal("Kaze-Sword-X").withStyle(Utils.styleOfKaze).withStyle(ChatFormatting.ITALIC));
