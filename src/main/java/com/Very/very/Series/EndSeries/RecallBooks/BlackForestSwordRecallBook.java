...
public class BlackForestSwordRecallBook extends Item {
    public BlackForestSwordRecallBook(Properties p_41383_) {
        super(p_41383_);
    }
    @Override
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag p_41424_) {
        ...
        components.add(Component.literal("暗黑森林的回忆").withStyle(Utils.styleOfBlackForest));
        components.add(Component.literal(" "));
        components.add(Component.literal("Recall-BlackForest").withStyle(Utils.styleOfEnd).withStyle(ChatFormatting.ITALIC));
        ...
        if (!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            if (Compute.RecallPlayerCheck((ServerPlayer) player)) {
                ...
                if (Utils.HuskRecall.RecallPlayer == null && Utils.HuskRecall.RecallCount == -1 && level.equals(level.getServer().getLevel(Level.END))) {
                    ...
                    Utils.HuskRecall.style = Utils.styleOfBlackForest;
                    Utils.HuskRecall.zoneName = "暗黑森林";
                }
                else {
