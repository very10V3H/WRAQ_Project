...
public class LightningRecallBook extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag p_41424_) {
        ...
        components.add(Component.literal("唤雷岛的回忆").withStyle(Utils.styleOfLightingIsland));
        components.add(Component.literal(" "));
        components.add(Component.literal("Recall-Lightning").withStyle(Utils.styleOfEnd).withStyle(ChatFormatting.ITALIC));
        ...
        if (!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            if (Compute.RecallPlayerCheck((ServerPlayer) player)) {
                ...
                if (Utils.LightningRecall.RecallPlayer == null && Utils.LightningRecall.RecallCount == -1 && level.equals(level.getServer().getLevel(Level.END))) {
                    ...
                    Utils.LightningRecall.style = Utils.styleOfLightingIsland;
                    Utils.LightningRecall.zoneName = "唤雷岛";
                }
                else {
