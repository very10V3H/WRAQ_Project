...
public class TabooManaArmor extends ArmorItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> components, TooltipFlag flag) {
        ...
        Compute.DescriptionPassive(components,Component.literal("禁忌秘法-涌动").withStyle(ChatFormatting.RESET).withStyle(style));
        components.add(Component.literal(" 根据你最近5s内消耗的").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.MaxMana("")).
                append(Component.literal("来提供等量").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                append(Compute.AttributeDescription.ManaPenetration("")));
        components.add(Component.literal(" 若你拥有至少一点:").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Component.literal("法术专精-力凝魔核").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfMana)).
                append(Component.literal("为你提供等同于层数 * 10%").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
    ...
    public record NearCostMana(int tick, int value) {
    }
    ...
    public static void storeCostToList(Player player, int value) {
