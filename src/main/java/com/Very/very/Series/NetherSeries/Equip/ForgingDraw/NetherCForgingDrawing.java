...
public class NetherCForgingDrawing extends Item {
    ...
    private final Item Material4 = Moditems.netherbonemeal.get();
    private final int Num1 = 64;
    private final int Num2 = 64;
    private final int Num3 = 64;
    private final int Num4 = 128;
    public NetherCForgingDrawing(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        components.add(Component.literal("·制作:灵魂沙之峡*1").withStyle(Utils.styleOfNether));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND))
        {
            ...
            if(ContainMaterial)
            {
                ...
                ItemStack itemStack = Moditems.NetherArmorChest.get().getDefaultInstance();
                itemStack.getOrCreateTagElement(Utils.MOD_ID).putInt("Number", ConfigTest.NetherArmorChest.get());
                Compute.FormatBroad(level,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                        Component.literal(player.getName().getString()+"成功锻造了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).append(itemStack.getDisplayName()).
                                append(Component.literal("这是维瑞阿契第"+ConfigTest.NetherArmorChest.get()+"件").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                append(Component.literal("灵魂沙之峡").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfNether)));
                ConfigTest.NetherArmorChest.set(ConfigTest.NetherArmorChest.get()+1);
