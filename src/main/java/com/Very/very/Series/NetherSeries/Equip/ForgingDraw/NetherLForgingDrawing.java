package com.Very.very.Series.NetherSeries.Equip.ForgingDraw;
...
public class NetherLForgingDrawing extends Item {
    private final Item Material1 = Moditems.GoldCoin.get();
    private final Item Material2 = Moditems.ruby.get();
    private final Item Material3 = Moditems.MineSoul1.get();
    private final Item Material4 = Moditems.NetherMagmaPower.get();
    private final int Num1 = 64;
    private final int Num2 = 64;
    private final int Num3 = 64;
    private final int Num4 = 128;
    public NetherLForgingDrawing(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        components.add(Component.literal("·制作:玄武岩之洲*1").withStyle(Utils.styleOfNether));
        ...
        components.add(Compute.MaterialRequirement(4,Material4.getDefaultInstance().getDisplayName(),Num4));
        components.add(Component.literal(" "));
        components.add(Component.literal("ForgingDrawing-Nether").withStyle(Utils.styleOfNether).withStyle(ChatFormatting.ITALIC));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND))
        {
            ...
            boolean ContainMaterial = Compute.ItemStackCheck(inventory, Material1, Num1)
                    ...
                    && Compute.ItemStackCheck(inventory, Material4, Num4);
            if(ContainMaterial)
            {
                ...
                Compute.ItemStackRemove(inventory,Material4,Num4);
                ...
                ItemStack itemStack = Moditems.NetherArmorLeggings.get().getDefaultInstance();
                itemStack.getOrCreateTagElement(Utils.MOD_ID).putInt("Number", ConfigTest.NetherArmorLeggings.get());
                Compute.FormatBroad(level,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                        Component.literal(player.getName().getString()+"成功锻造了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).append(itemStack.getDisplayName()).
                                append(Component.literal("这是维瑞阿契第"+ConfigTest.NetherArmorLeggings.get()+"件").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                append(Component.literal("玄武岩之洲").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfNether)));
                ConfigTest.NetherArmorLeggings.set(ConfigTest.NetherArmorLeggings.get()+1);
            ...
            else
            {
                ...
                if(Compute.ItemStackCount(inventory,Material4) < Num4)
                {
                    Compute.FormatMSGSend(player,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                            Component.literal("缺少:").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                                    append(Material4.getDefaultInstance().getDisplayName()).
                                    append(Component.literal("*"+(Num4-Compute.ItemStackCount(inventory,Material4)))));
                }
            }
        }
