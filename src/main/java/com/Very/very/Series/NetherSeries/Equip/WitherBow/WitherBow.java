package com.Very.very.Series.NetherSeries.Equip.WitherBow;
...
public class WitherBow extends PickaxeItem {
    ...
    {
        Style style = Utils.styleOfWither;
        Compute.ForgingHoverName(stack,Component.literal("凋零长弓" + WitherBowAttributes.Number[Num]).withStyle(ChatFormatting.RESET).withStyle(style).withStyle(ChatFormatting.BOLD));
        ...
        Compute.DescriptionPassive(components,Component.literal("凋零增幅").withStyle(ChatFormatting.RESET).withStyle(style));
        components.add(Component.literal("箭矢命中后:"));
        components.add(Component.literal("损失").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.RED).
                append(Compute.AttributeDescription.MaxHealth("7%")));
        components.add(Component.literal("获得").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GREEN).
...
/*    @Override
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        Compute.BowAttack(player,Utils.BowNumMap.get(StringUtils.BowNameString.WitherBow));
