...
public class NetherHForgingDrawing extends Item {
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        components.add(Component.literal("·制作:要塞之核*1").withStyle(Utils.styleOfNether));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if(!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND))
        {
            ...
            if(ContainMaterial)
            {
                ...
                ItemStack itemStack = Moditems.NetherArmorHelmet.get().getDefaultInstance();
                itemStack.getOrCreateTagElement(Utils.MOD_ID).putInt("Number", ConfigTest.NetherArmorHelmet.get());
                Compute.FormatBroad(level,Component.literal("锻造").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.GRAY),
                        Component.literal(player.getName().getString()+"成功锻造了").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).append(itemStack.getDisplayName()).
                                append(Component.literal("这是维瑞阿契第"+ConfigTest.NetherArmorHelmet.get()+"件").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                                append(Component.literal("要塞之核").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfNether)));
                ConfigTest.NetherArmorHelmet.set(ConfigTest.NetherArmorHelmet.get()+1);
