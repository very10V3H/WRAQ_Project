...
public class NetherGemPieceBag4 extends Item{
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if (!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            ...
            Compute.RandomAttributeProvider(itemStack1,1,2,player);
            Compute.RandomAttributeProvider(itemStack1,1,1.95,player);
            Compute.RandomAttributeProvider(itemStack1,1,1.9,player);
            Compute.RandomAttributeProvider(itemStack1,1,1.85,player);
            Compute.RandomAttributeProvider(itemStack1,1,1.8,player);
            Compute.RandomAttributeProvider(itemStack1,1,1.75,player);
            Compute.RandomAttributeProvider(itemStack1,1,1.7,player);
            Compute.RandomAttributeProvider(itemStack1,1,1.65,player);
