...
public class NetherGemPieceBag1 extends Item{
    public NetherGemPieceBag1(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag flag) {
        ...
        components.add(Component.literal("换取低阶下界征讨者胸章。").withStyle(Style.EMPTY.withColor(TextColor.parseColor("#a2001b"))));
        components.add(Component.literal(" "));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if (!level.isClientSide && interactionHand.equals(InteractionHand.MAIN_HAND)) {
            ...
            Compute.RandomAttributeProvider(itemStack1,1,0.9,player);
            Compute.RandomAttributeProvider(itemStack1,1,0.8,player);
            Compute.RandomAttributeProvider(itemStack1,1,0.7,player);
            Compute.RandomAttributeProvider(itemStack1,1,0.6,player);
            Compute.RandomAttributeProvider(itemStack1,1,0.5,player);
            Compute.RandomAttributeProvider(itemStack1,1,0.5,player);
            Compute.RandomAttributeProvider(itemStack1,1,0.5,player);
