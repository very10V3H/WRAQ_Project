...
public class SoraVanilla2 extends Item implements ICurioItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> components, TooltipFlag flag) {
        ...
        Compute.DescriptionPassive(components,Component.literal("初始之火的继承者").withStyle(ChatFormatting.RESET).withStyle(style));
        components.add(Component.literal(" 你的").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Component.literal("法术技能").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfMana)).
                append(Component.literal("会进行双重施法，每释放一个技能扣除").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                append(Compute.AttributeDescription.Health("10%当前")).
                append(Component.literal("，并为你提供持续3s的等量").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
        ...
        Compute.DescriptionPassive(components,Component.literal("血缘之末—洛斯里克的圣王").withStyle(ChatFormatting.RESET).withStyle(style));
        components.add(Component.literal(" 当你的拥有低于某一阈值的生命值时，为你提供法术强度:").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE));
        ...
        components.add(Component.literal(" 在低于").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.Health("25%")).
                append(Component.literal("时，提供").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)).
                append(Component.literal("80%伤害提升").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfMana)));
        components.add(Component.literal(" 每一代薪王都会继承上一代的诅咒，继续燃烧自己的灵魂來传承火，" +
                "一个强大的灵魂的死亡才能够换来这个世界的苟延残喘，我们到底该何去何从呢？我们是要熄灭这火焰，" +
                "然后进入无人知晓的黑暗时代还是继续牺牲灵魂，来使这世界苟延残喘下去。").withStyle(ChatFormatting.RESET).withStyle(style));
        Compute.DescriptionDash(components, ChatFormatting.WHITE, style, ChatFormatting.WHITE);
    ...
    public static void ReleaseRecord(Player player, int index) {
        ...
        Compute.PlayerHealthDecrease(player,player.getHealth() * 0.1,
                Component.literal(" 被黑暗灵魂吞噬了").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfCastle),0.2);
    }
