...
public class BlackFeisaCurios2 extends Item implements ICurioItem {

    public static Player Player;
    public static boolean IsOn;

    public BlackFeisaCurios2(Properties p_41383_) {
        super(p_41383_);
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> components, TooltipFlag flag) {
        ...
        Compute.DescriptionPassive(components, Component.literal("黑手").withStyle(ChatFormatting.RESET).withStyle(style));
        components.add(Component.literal(" 每过2s，若周围有敌人，则对周围所有敌人造成").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.ManaDamageValue("400%")).
                append(Component.literal("，并提供1层充能").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE)));
        components.add(Component.literal(" 当充能满时，发射激光，受到激光影响的目标每0.5s受到").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
        ...
        components.add(Component.literal(" 敢不敢跟我比划比划?").withStyle(ChatFormatting.RESET).withStyle(style));
        Compute.DescriptionDash(components, ChatFormatting.WHITE, style, ChatFormatting.WHITE);
        components.add(Component.literal(" 崇高的敬意化作一枚神鹰黑手哥，授予对维瑞阿契做出了杰出贡献的 - Black_FeiSa_").withStyle(ChatFormatting.AQUA));
    ...
    public static void Laser(Player player) {
        ...
        if (LasetTick > TickCount) {
            Compute.Laser(player,ModParticles.SNOW.get(), Compute.XpStrengthAPDamage(player,0.5),10);
        }
    }
