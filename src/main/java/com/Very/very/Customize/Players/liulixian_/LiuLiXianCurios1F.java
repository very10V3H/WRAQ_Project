...
public class LiuLiXianCurios1F {
    ...
    public static void Skill2(Player player) {
        ...
        Compute.PlayerShieldProvider(player,180,player.getMaxHealth() * 0.5);
    }
        ...
        if (Skill8Tick > TickCount) {
            ...
            playerList.forEach(player1 -> {
                if (player1.distanceTo(player) < 6) {
                    Compute.PlayerHealthDecrease(player1,player1.getHealth() * 0.005,
                            Component.literal(" 被琉璃仙法吞噬了。").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSakura));
                }
            });
        }
    }
    ...
    public static double LiuLiXianDamageCompute(Player player, double damage) {
        ...
        if (UltimateSkillTick > TickCount) {
            if (damage > player.getMaxHealth() * 0.1) Compute.FormatMSGSend(player,Component.literal("墨仙").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSakura),
            ...
            if (damage > player.getMaxHealth() * 0.2) Compute.FormatMSGSend(player,Component.literal("墨仙").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSakura),
            ...
            if (damage > player.getMaxHealth() * 0.3) Compute.FormatMSGSend(player,Component.literal("墨仙").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfSakura),
    ...
    public static void Tick(Player player) {
        if (IsLiuLiXian(player)) {
            int TickCount = player.getServer().getTickCount();
            if (TickCount % 40 == 0) Passive(player);
            Skill1Shoot(player);
