...
public class YxwgCurios1 extends Item implements ICurioItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> components, TooltipFlag flag)
    {
        ...
        Compute.DescriptionPassive(components,Component.literal("连诛").withStyle(ChatFormatting.RESET).withStyle(style));
        components.add(Component.literal(" 箭矢命中目标时，有33%概率触发多重连射效果。在0.4s内射出").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Component.literal("4支箭矢").withStyle(ChatFormatting.RESET).withStyle(Utils.styleOfFlexible)));
        Compute.DescriptionPassive(components,Component.literal("追猎").withStyle(ChatFormatting.RESET).withStyle(style));
        components.add(Component.literal(" 箭矢命中目标时，若未触发多重连射，获得").withStyle(ChatFormatting.RESET).withStyle(ChatFormatting.WHITE).
                append(Compute.AttributeDescription.AttackDamage("25%总")));
        components.add(Component.literal(" 力量就像是与生俱来的一样，一点一点的击碎敌人").withStyle(ChatFormatting.RESET).withStyle(style));
        Compute.DescriptionDash(components,ChatFormatting.WHITE,style,ChatFormatting.WHITE);
